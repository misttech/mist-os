...
struct Opt {
    ...
    /// external subtool manifest to use with ffx.
    /// otherwise, ffx scans configured locations for subtools.
    #[argh(option)]
    subtool_manifest: Option<PathBuf>,
...
// TODO(https://fxbug.dev/42148459): This is less of an allowlist and more a
// default list of tools when opt.cmd_list is empty.
    ...
    if let Some(tardir) = opt.archive_path {
        ...
        if let Some(depfile_path) = opt.depfile {
            ...
            for cmd_path in cmd_paths {
                info!("depfile: {output_filename}: {cmd_path:?}");
                write!(f, "{output_filename}: {}\n", cmd_path.display())?;
            }
            if let Some(subtool_manifest) = &opt.subtool_manifest {
                for ffx_file in ffx_doc::list_subtool_files(subtool_manifest) {
                    info!("depfile: {output_filename}: {ffx_file:?}");
                    write!(f, "{output_filename}: {}\n", ffx_file.display())?;
                }
            }
        }
