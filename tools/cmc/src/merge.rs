...
pub(crate) fn merge(
    ...
    let mut writer: Box<dyn Write> = if let Some(output_path) = &output {
        ...
        Box::new(BufWriter::new(
            fs::OpenOptions::new().create(true).truncate(true).write(true).open(output_path)?,
        ))
    } else {
...
struct HeaderFormatter<'a> {
    input_files: &'a [PathBuf],
}

impl Display for HeaderFormatter<'_> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let Self { input_files } = self;
        writeln!(f, "// This file was generated by cmc merge.")?;
        writeln!(f, "//")?;
        writeln!(f, "// The following source files were merged:")?;
...
mod tests {
    ...
    fn test_merge_cml() {
        ...
        merge(filenames.clone(), Some(output_file_path.clone()), None, None)
        ...
        let header = HeaderFormatter { input_files: &filenames[..] };
        assert_eq!(buffer, format!("{}{:#}", header, expected_json));
    }
    ...
    fn test_merge_fromfile() {
        ...
        merge(filenames.clone(), Some(output_file_path.clone()), Some(fromfile_path), None)
            .expect("failed to merge");
        ...
        let header_files = input.iter().map(|(name, _)| name.clone()).collect::<Vec<_>>();
        let header = HeaderFormatter { input_files: &header_files[..] };
        assert_eq!(buffer, format!("{}{:#}", header, expected_json));
    }
}
