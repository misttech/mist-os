...
const (
	...
	// The directory is unspecified.
	ninjatraceJSONName = "ninjatrace.json"
...
type ninjaDebugFileSet struct {
	// logPath is an absolute path to a .ninja_log
	logPath string
	// depsPath is an absolute path to a .ninja_deps
	depsPath string
	// graphPath is an absolute path ninja graphviz file.
	// Deprecated: Keeping this exposed is only transitional.
	graphPath string
	// compdbPath is an absolute path to a ninja compile commands file.
	// Deprecated: Keeping this exposed is only transitional.
	compdbPath string
...
func getNinjaDebugFiles(ctx context.Context, r ninjaRunner, buildDir string, targets []string, ninjaLog string, ninjatraceToolPath string, buildstatsToolPath string) (*ninjaDebugFileSet, error) {
	...
	// The graph and compdb files are only needed temporarily to generate the trace JSON.
	graph := filepath.Join(subBuildDirAbs, ninjaGraphName)
	if err := ninjaGraph(ctx, r, targets, graph); err != nil {
		return debugFiles, err
	}
	debugFiles.graphPath = graph

	compdb := filepath.Join(subBuildDirAbs, ninjaCompdbName)
	if err := ninjaCompdb(ctx, r, compdb); err != nil {
		return debugFiles, err
	}
	debugFiles.compdbPath = compdb

	ninjatraceJSON := filepath.Join(subBuildDirAbs, ninjatraceJSONName)
	if err := runNinjatrace(ctx, r.runner, ninjatraceToolPath, ninjaLog, compdb, graph, ninjatraceJSON); err != nil {
		return debugFiles, err
	}
	debugFiles.tracePath = ninjatraceJSON

	buildstatsJSON := filepath.Join(subBuildDirAbs, buildstatsJSONName)
	if err := runBuildstats(ctx, r.runner, buildstatsToolPath, ninjaLog, compdb, graph, buildstatsJSON); err != nil {
		return debugFiles, err
	}
...
func collectNinjaArtifacts(buildDir string, ninjaDebugFiles []*ninjaDebugFileSet, fileExists func(string) bool) []string {
	...
	for _, ndf := range ninjaDebugFiles {
		files := []string{ndf.logPath, ndf.depsPath, ndf.tracePath, ndf.statsPath}
		for _, f := range files {
			if fileExists(f) {
