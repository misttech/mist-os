...
class CoordinatorClientWithoutListenerTest : public ::testing::Test {
 private:
...
class MockCoordinatorListener
 ...
 public:
  ...
  void OnDisplaysChanged(OnDisplaysChangedRequestView request,
    ...
    for (fuchsia_hardware_display_types::wire::DisplayId fidl_id : request->removed) {
      latest_removed_display_ids_.push_back(ToDisplayId(fidl_id));
    }
  }
  ...
  void OnVsync(OnVsyncRequestView request, OnVsyncCompleter::Sync& completer) override {
    ...
    latest_applied_config_stamp_ = ToConfigStamp(request->applied_config_stamp);
  }
  ...
  ConfigStamp latest_applied_config_stamp() const { return latest_applied_config_stamp_; }
 ...
 private:
  ...
  ConfigStamp latest_applied_config_stamp_ = kInvalidConfigStamp;
};

TEST_F(CoordinatorClientWithoutListenerTest, ClientVSyncOk) {
  ...
  ASSERT_OK(
      clientproxy.InitForTesting(std::move(server_end), /*coordinator_listener_client_end=*/{}));
...
TEST_F(CoordinatorClientWithoutListenerTest, ClientVSyncPeerClosed) {
  fdf_testing::DriverRuntime driver_runtime;
...
TEST_F(CoordinatorClientWithoutListenerTest, ClientVSyncNotSupported) {
  fdf_testing::DriverRuntime driver_runtime;
...
TEST_F(CoordinatorClientWithoutListenerTest, ClientMustDrainPendingStamps) {
  fdf_testing::DriverRuntime driver_runtime;
