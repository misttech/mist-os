...
DisplayPll* DisplayPllManager::SetDdiPllConfig(DdiId ddi_id, bool is_edp,
  ...
  fdf::trace("Configuring PLL for DDI {} - SSC {}, DDI clock {} kHz, DisplayPort {}, HDMI {}",
  ...
  if (ddi_to_dpll_it != ddi_to_dpll_.end()) {
    ...
    if (pll->config() == desired_config) {
      fdf::warn("SetDdiPllConfig() will unnecessarily reset the PLL for DDI {}", ddi_id);
    }
    ResetDdiPll(ddi_id);
  }
  ...
  if (!best_dpll) {
    fdf::error("Failed to allocate DPLL to DDI {} - {} kHz {} DisplayPort: {} HDMI: {}", ddi_id,
  ...
  fdf::debug(
      "Assigning DPLL {} to DDI {} - {} kHz {} DisplayPort: {} HDMI: {}", best_dpll->name(), ddi_id,
      desired_config.ddi_clock_khz, desired_config.spread_spectrum_clocking ? "SSC" : "no SSC",
      desired_config.admits_display_port ? "yes" : "no", desired_config.admits_hdmi ? "yes" : "no");

  if (ref_count_[best_dpll] > 0 || best_dpll->Enable(desired_config)) {
    if (!SetDdiClockSource(ddi_id, best_dpll->pll_id())) {
      fdf::error("Failed to map DDI {} to DPLL ({})", ddi_id, best_dpll->name());
      return nullptr;
    }
...
bool DisplayPll::Enable(const DdiPllConfig& pll_config) {
  fdf::trace("Configuring PLL {}: SSC {}, DDI clock {} kHz, DisplayPort {}, HDMI {}", pll_id(),
  ...
  if (!config_.IsEmpty()) {
    fdf::error("Enable(): PLL {} already enabled!", name());
    return false;
  }
  ...
  if (success) {
    config_ = pll_config;
    fdf::trace("Enabled DPLL {}: SSC {}, DDI clock {} kHz, DisplayPort {}, HDMI {}", pll_id(),
               pll_config.spread_spectrum_clocking ? "yes" : "no", pll_config.ddi_clock_khz,
               pll_config.admits_display_port ? "yes" : "no",
               pll_config.admits_hdmi ? "yes" : "no");

  } else {
    fdf::error("Failed to enable DPLL {}: SSC {}, DDI clock {} kHz, DisplayPort {}, HDMI {}",
               pll_id(), pll_config.spread_spectrum_clocking ? "yes" : "no",
               pll_config.ddi_clock_khz, pll_config.admits_display_port ? "yes" : "no",
               pll_config.admits_hdmi ? "yes" : "no");
  }
  return success;
}
...
bool DisplayPll::Disable() {
  fdf::trace("Disabling PLL {}", pll_id());
  if (config_.IsEmpty()) {
    fdf::info("DoDisable(): PLL {} already disabled", name());
    return true;
  }
  ...
  if (success) {
    config_ = {};
    fdf::trace("Disabled PLL {}", pll_id());
  } else {
    fdf::error("Failed to disable PLL {}", pll_id());
  }
  return success;
}
...
bool DisplayPllManager::ResetDdiPll(DdiId ddi_id) {
  ...
  if (!ResetDdiClockSource(ddi_id)) {
    fdf::error("Failed to unmap DPLL ({}) for DDI {}", dpll->name(), ddi_id);
    return false;
  }
...
bool DpllSkylake::DoEnable(const DdiPllConfig& pll_config) {
  ...
  if (!display::PollUntil(
          ...
          zx::msec(1), 5)) {
    fdf::error("Skylake DPLL {} failed to lock after 5ms!", pll_id());
    return false;
  }
...
bool DpllSkylake::ConfigureForDisplayPort(const DdiPllConfig& pll_config) {
  ...
  fdf::trace("Configuring Skylake DPLL {}: DisplayPort, link rate {} Mbps", pll_id(),
...
bool DpllSkylake::ConfigureForHdmi(const DdiPllConfig& pll_config) {
  ...
  fdf::trace("Configuring DPLL {}: HDMI DCO frequency={} dividers P*Q*K={}*{}*{} Center={} Mhz",
...
DdiPllConfig DpllManagerSkylake::LoadState(DdiId ddi_id) {
  ...
  if (dpll_ddi_map.ddi_clock_disabled(ddi_id)) {
    fdf::trace("Loaded DDI {} DPLL state: DDI clock disabled", ddi_id);
    return DdiPllConfig{};
  }
  ...
  if (!dpll_enable.pll_enabled()) {
    fdf::trace("Loaded DDI {} DPLL {} state: DPLL disabled", ddi_id, pll_id);
    return DdiPllConfig{};
  }
  ...
  if (uses_hdmi_mode) {
    ...
    fdf::trace(
        "Loaded DDI {} DPLL {} state: HDMI no SSC DCO frequency={} kHz divider P*Q*K={}*{}*{} "
        "Center={} Mhz",
  ...
  if (ddi_frequency_mhz == 0) {
    fdf::error("DPLL {} has invalid DisplayPort DDI clock. DPLL_CTRL1 value: {:x}", pll_id,
  ...
  fdf::trace("Loaded DDI {} DPLL {} state: DisplayPort {} {} kHz (link rate {} Mbps)", ddi_id,
...
bool DisplayPllTigerLake::DoEnable(const DdiPllConfig& pll_config) {
  ...
  fdf::trace("Configuring PLL {}: DCO frequency={} dividers P*Q*K={}*{}*{} Center={} Mhz", pll_id(),
             dco_config.frequency_khz, divider_config.p0_p_divider, divider_config.p1_q_divider,
  ...
  if (!display::PollUntil([&] { return dpll_enable.ReadFrom(mmio_space_).powered_on_tiger_lake(); },
    ...
    fdf::error("DPLL {} power up failure!", pll_id());
    return false;
  }
  ...
  if (!display::PollUntil(
    ...
    fdf::error("DPLL {} lock failure! Failed to lock after 500us", pll_id());
    return false;
  }
...
bool DekelPllTigerLake::DoDisable() {
  ...
  if (!display::PollUntil(
          ...
          zx::usec(1), 50)) {
    fdf::error("Dekel PLL {}: Cannot disable PLL", name());
  }
  ...
  if (!display::PollUntil([&] { return !pll_enable.ReadFrom(mmio_space_).powered_on_tiger_lake(); },
                          zx::usec(1), 10)) {
    fdf::error("Dekel PLL {}: Cannot disable PLL power", name());
  }
...
bool DekelPllTigerLake::EnableHdmi(const DdiPllConfig& pll_config) {
  ...
  fdf::error("Dekel PLL {}: EnableHdmi: Not implemented", name());
  return false;
}
...
bool DekelPllTigerLake::EnableDp(const DdiPllConfig& pll_config) {
  ...
  if (!display::PollUntil([&] { return pll_enable.ReadFrom(mmio_space_).powered_on_tiger_lake(); },
                          zx::usec(1), 10)) {
    fdf::error("Dekel PLL {}: Cannot enable PLL power", name());
    return false;
  }
  ...
  switch (display_port_link_rate_mbps) {
    ...
    default: {
      fdf::error("Unsupported DP link rate: {} Mbps", display_port_link_rate_mbps);
      return false;
    }
  }
  ...
  if (!display::PollUntil(
          [&] { return pll_enable.ReadFrom(mmio_space_).pll_locked_tiger_lake_and_lcpll1(); },
          zx::usec(1), 900)) {
    fdf::error("Dekel PLL ({}): Cannot enable PLL", name());
    return false;
  }
...
bool DpllManagerTigerLake::SetDdiClockSource(DdiId ddi_id, PllId pll_id) {
  switch (pll_id) {
    ...
    case PllId::DPLL_2:
      ...
      fdf::error("SetDdiClockSource() does not support DPLL 2 (for Thunderbolt) yet");
      return false;

    default:
      fdf::error("SetDdiClockSource() does not support DPLL {} yet", pll_id);
      return false;
  }
}
...
DdiPllConfig DpllManagerTigerLake::LoadStateForComboDdi(DdiId ddi_id) {
  ...
  if (ddi_clock_config.ddi_clock_disabled(ddi_id)) {
    fdf::trace("Loaded DDI {} DPLL config: DDI clock disabled", ddi_id);
    return DdiPllConfig{};
  }
  ...
  if (pll_id == PllId::DPLL_INVALID) {
    fdf::warn(
        "Invalid DDI {} DPLL config: Invalid clock source DPLL! DDI Clock Config register: {:x}",
        ddi_id, ddi_clock_config.reg_value());
    return DdiPllConfig{};
  }
  if (pll_id == PllId::DPLL_2) {
    fdf::warn(
        "Invalid DDI {} DPLL config: clock source is DPLL 2, but DPLL2 reserved for Thunderbot.",
        ddi_id);
    return DdiPllConfig{};
  }
  ...
  auto dpll_enable = registers::PllEnable::GetForTigerLakeDpll(pll_id).ReadFrom(mmio_space_);
  if (!dpll_enable.pll_enabled()) {
    fdf::trace("Loaded DDI {} DPLL {} config: DPLL disabled", ddi_id, pll_id);
    return DdiPllConfig{};
  }
  ...
  fdf::trace(
      "Loaded DDI {} DPLL {} dividers: early lock {}, true lock {}, AFC start point {}, "
      "feedback clock retiming {}, loop filter - integral 2^(-{}) proportional 2^(1-{}) gain 2^{} "
      "pre-divider {}, post-divider (M2) {}",
  ...
  if (dpll_dco_dividers.reference_clock_select() !=
    ...
    fdf::error(
        "Loaded DDI {} DPLL {} config: DPLL uses genlock clock reference {}. Genlock not supported!",
  ...
  fdf::trace(
      "Loaded DDI {} DPLL {} config: {} DDI clock {} kHz DCO frequency={} kHz divider P*Q*K={}*{}*{}",
...
DdiPllConfig DpllManagerTigerLake::LoadStateForTypeCDdi(DdiId ddi_id) {
  ...
  fdf::warn("LoadTypeCPllState: DDI {} has invalid DisplayPort bit rate: {} KHz", ddi_id,
            bit_rate_khz);
  return DdiPllConfig{};
}
