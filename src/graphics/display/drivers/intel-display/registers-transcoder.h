...
#ifndef SRC_GRAPHICS_DISPLAY_DRIVERS_INTEL_DISPLAY_REGISTERS_TRANSCODER_H_
#define SRC_GRAPHICS_DISPLAY_DRIVERS_INTEL_DISPLAY_REGISTERS_TRANSCODER_H_
...
class TranscoderDdiControl : public hwreg::RegisterBase<TranscoderDdiControl, uint32_t> {
 public:
  ...
  // DDI A.
  std::optional<intel_display::DdiId> ddi_kaby_lake() const {
    ...
    return static_cast<intel_display::DdiId>(ddi_index);
  }
  ...
  std::optional<intel_display::DdiId> ddi_tiger_lake() const {
    // The cast is lossless because `ddi_select_subtle()` is a 4-bit field.
    const int8_t ddi_select_raw_value = static_cast<int8_t>(ddi_select_subtle());
    if (ddi_select_raw_value == 0) {
      return std::nullopt;
  ...
  TranscoderDdiControl& set_ddi_kaby_lake(std::optional<intel_display::DdiId> ddi_id) {
    ...
    ZX_DEBUG_ASSERT_MSG(*ddi_id != intel_display::DdiId::DDI_A,
  ...
  intel_display::TranscoderId port_sync_primary_transcoder_kaby_lake() const {
    ...
    if (raw_port_sync_primary_select == 0) {
      return intel_display::TranscoderId::TRANSCODER_EDP;
    }
    ...
    return static_cast<intel_display::TranscoderId>(intel_display::TranscoderId::TRANSCODER_A +
  ...
  TranscoderDdiControl& set_port_sync_primary_kaby_lake(intel_display::TranscoderId transcoder_id) {
    if (transcoder_id == intel_display::TranscoderId::TRANSCODER_EDP) {
    ...
    return set_port_sync_primary_transcoder_select_kaby_lake(
        (transcoder_id - intel_display::TranscoderId::TRANSCODER_A) + 1);
  }
  ...
  static auto GetForKabyLakeTranscoder(intel_display::TranscoderId transcoder_id) {
    if (transcoder_id == intel_display::TranscoderId::TRANSCODER_EDP) {
...
class TranscoderClockSelect : public hwreg::RegisterBase<TranscoderClockSelect, uint32_t> {
 public:
  ...
  std::optional<intel_display::DdiId> ddi_clock_kaby_lake() const {
  ...
  //
  // This field must not be changed while the transcoder is enabled.
  std::optional<intel_display::DdiId> ddi_clock_tiger_lake() const {
  ...
  TranscoderClockSelect& set_ddi_clock_kaby_lake(std::optional<intel_display::DdiId> ddi_id) {
    ZX_DEBUG_ASSERT_MSG(!ddi_id.has_value() || ddi_id != intel_display::DdiId::DDI_A,
                        "DDI A cannot be explicitly connected to a transcoder");
  ...
  static auto GetForTranscoder(intel_display::TranscoderId transcoder_id) {
    ZX_ASSERT(transcoder_id >= intel_display::TranscoderId::TRANSCODER_A);
 ...
 private:
  static int8_t RawDdiClockSelect(std::optional<intel_display::DdiId> ddi) {
    if (!ddi.has_value()) {
      return 0;
    }
    // The cast is lossless because DDI indices fit in 4 bits.
    const int8_t ddi_index = static_cast<int8_t>(*ddi - intel_display::DdiId::DDI_A);
...
class TranscoderChicken : public hwreg::RegisterBase<TranscoderChicken, uint32_t> {
 public:
  ...
  static auto GetForKabyLakeDdi(intel_display::DdiId ddi_id) {
    ...
    ZX_ASSERT(ddi_id <= intel_display::DdiId::DDI_D);
...
class TranscoderRegs {
 public:
  explicit TranscoderRegs(intel_display::TranscoderId transcoder_id)
      : transcoder_id_(transcoder_id),
        offset_(transcoder_id_ == intel_display::TranscoderId::TRANSCODER_EDP
                    ? 0xf000
 ...
 private:
  ...
  intel_display::TranscoderId transcoder_id_;
  uint32_t offset_;
};
}  // namespace registers

#endif  // SRC_GRAPHICS_DISPLAY_DRIVERS_INTEL_DISPLAY_REGISTERS_TRANSCODER_H_
