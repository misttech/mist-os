...
TEST(MagmaUtil, ZxStatus) {
  ...
  EXPECT_EQ(magma::ToZxStatus(MAGMA_STATUS_CONSTRAINTS_INTERSECTION_EMPTY), ZX_ERR_NOT_SUPPORTED);
  EXPECT_EQ(magma::ToZxStatus(MAGMA_STATUS_TOO_MANY_GROUP_CHILD_COMBINATIONS), ZX_ERR_OUT_OF_RANGE);
  EXPECT_EQ(MAGMA_STATUS_ALIAS_FOR_LAST, MAGMA_STATUS_TOO_MANY_GROUP_CHILD_COMBINATIONS)
      << "test needs updating";
}
#endif
