...
impl ContainerBuffer {
    ...
    pub fn cursor(&self, mode: StreamMode) -> Option<Cursor> {
        let inner = self.shared_buffer.inner.lock();
        let Some(container) = inner.containers.get(self.container_id) else {
        ...
        let (index, next_id, end_id) = match mode {
            ...
            StreamMode::Subscribe => (inner.head, container.msg_ids.end, None),
    ...
    pub fn terminate(&self) {
        ...
        if let Some(container) = inner.containers.get_mut(self.container_id) {
            ...
            if container.msg_ids.start == container.msg_ids.end {
