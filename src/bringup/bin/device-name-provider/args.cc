...
int ParseArgs(int argc, char** argv, const device_name_provider_config::Config& config,
  ...
  *out = DeviceNameProviderArgs();
  ...
  // First parse from kernel args and structured configuration, then use use cmdline args as
  // overrides.
  ...
  if (string_resp.ok()) {
    ...
    out->nodename = std::string{values[0].data(), values[0].size()};
    namegen_str = std::string{values[1].data(), values[1].size()};
  }
