...
async fn main_async() -> Result<(), Error> {
    let state_and_out_dir = match fuchsia_runtime::take_startup_handle(
        fuchsia_runtime::HandleInfo::new(fuchsia_runtime::HandleType::EscrowedDictionary, 0),
    ) {
        Some(dictionary) => {
            resume_from_idle_stop(fsandbox::DictionaryRef { token: dictionary.into() })
                .await
                .context("resume_from_idle_stop")?
        }
        None => first_run().await.context("first_run")?,
    };
    if let Some((state, outgoing_dir)) = state_and_out_dir {
        ...
        let (_lifecycle_request_stream, lifecycle_control_handle) =
            lifecycle.into_stream_and_control_handle();
        let () = lifecycle_control_handle
...
// Returns state and out dir if stopped on idle.
async fn first_run(
) -> Result<Option<(EscrowState, ::fidl::endpoints::ServerEnd<fio::DirectoryMarker>)>, Error> {
    ...
    let commit_fut = async move {
        if let Err(e) = metadata::put_metadata_in_happy_state(
            &boot_manager,
            &p_internal,
            unblocker,
    ...
    let mut service_fut = fs.for_each_concurrent(None, |item| async {
    ...
    let () = service_fut.await;
    let out_dir = out_dir.lock().take();
...
// Returns state and out dir if stopped on idle.
async fn resume_from_idle_stop(
    escrowed_state: fsandbox::DictionaryRef,
) -> Result<Option<(EscrowState, ::fidl::endpoints::ServerEnd<fio::DirectoryMarker>)>, Error> {
    ...
    Ok(out_dir.map(|d| (EscrowState { p_internal, p_external, frozen_inspect }, d.into())))
}
