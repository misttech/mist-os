...
#[argh(
    subcommand,
    name = "console",
    example = "starnix console [-e ENV=VAL -e ...] program [argument ...]",
...
pub async fn starnix_console(command: &StarnixConsoleCommand) -> Result<()> {
    if !termion::is_tty(&std::io::stdout()) {
        bail!(
            "starnix console must be run in a tty. \
    ...
    if command.argv.is_empty() {
        bail!(
            "Please specify a program to run.\n\
               Examples:\n\
               starnix console /bin/bash\n\
               starnix console /bin/ls -l /\n\
               Use starnix console --help for more information."
        );
    }
    let controller = connect_to_contoller(command.moniker.clone()).await?;
