...
class StaticFidlClientTests(unittest.IsolatedAsyncioTestCase):
    """Fidl Client tests."""

    async def test_read_and_decode_staged_message(self) -> None:
        channel = Mock()
        channel.__class__ = Channel  # type: ignore
        ...
        proxy.channel_waker.handle_ready_queues[0] = asyncio.Queue()  # type: ignore
        proxy.channel_waker.handle_ready_queues[0].put_nowait(0)
        proxy._decode = Mock()  # type: ignore
    ...
    def test_queue_wrapper_put_nowait(self) -> None:
        ...
        loop.close()
        self.assertTrue(queue.loop.is_closed())  # type: ignore
        inner = queue.queue
