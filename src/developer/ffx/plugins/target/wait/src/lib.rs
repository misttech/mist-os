...
#[derive(FfxTool)]
pub struct WaitTool<T: DeviceWaiter + fho::TryFromEnv> {
    #[command]
    cmd: WaitCommand,
    env: EnvironmentContext,
    waiter: T,
...
fho::embedded_plugin!(WaitTool<DeviceWaiterImpl>);

#[async_trait(?Send)]
impl<T: DeviceWaiter + fho::TryFromEnv> FfxMain for WaitTool<T> {
...
impl<T: DeviceWaiter + fho::TryFromEnv> WaitTool<T> {
    async fn wait_impl(&self) -> Result<()> {
        ...
        let duration = if self.cmd.timeout > 0 {
            Some(Duration::from_secs(self.cmd.timeout as u64))
        } else {
            None
        };
...
mod test {
    ...
    async fn test_success() {
        ...
        let writer = <WaitTool<MockDeviceWaiter> as FfxMain>::Writer::new_test(
        ...
        <WaitTool<MockDeviceWaiter> as FfxMain>::Writer::verify_schema(&json).expect(&err)
    }
