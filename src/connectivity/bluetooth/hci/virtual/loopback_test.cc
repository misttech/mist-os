...
class LoopbackTest : public ::testing::Test,
 ...
 public:
  void SetUp() override {
    ...
    auto add_child_cb = [&](fuchsia_driver_framework::wire::NodeAddArgs) { add_child_cb_count++; };
    zx_status_t status = loopback_device_.Initialize(std::move(loopback_channel_device_end),
                                                     "loopback", std::move(add_child_cb));
    ...
    // There's no driver instance to set up the logger for this test, so create a driver logger for
    // this test.
    InitializeLogger();
  ...
  const std::vector<fidl::Event<fhb::Snoop::OnObservePacket>>& snoop_packets() const {
    return snoop_packets_;
  }

  const std::vector<fidl::Event<fhb::Snoop::OnDroppedPackets>>& dropped_snoop_packets() {
    return dropped_snoop_packets_;
  }

  // Packets received on the loopback channel (i.e. packets sent by LoopbackDevice).
  const std::vector<std::vector<uint8_t>>& sent_packets() const { return sent_packets_; }

  const std::vector<fhb::ReceivedPacket>& received_packets() const { return received_packets_; }

 private:
  // fidl::AsyncEventHandler<fhb::HciTransport> overrides:
  void OnReceive(
      ::fidl::Event<::fuchsia_hardware_bluetooth::HciTransport::OnReceive>& event) override {
  ...
  void OnDroppedPackets(fidl::Event<fhb::Snoop::OnDroppedPackets>& event) override {
    dropped_snoop_packets_.emplace_back(std::move(event));
  }
  void handle_unknown_event(fidl::UnknownEventMetadata<fhb::Snoop> metadata) override {}
  ...
  void OpenHciTransport() {
    ...
    vendor_client_->OpenHciTransport().Then(
        [this](fidl::Result<fhb::Vendor::OpenHciTransport>& result) {
          ...
          hci_client_.Bind(std::move(result.value().channel()), dispatcher(),
                           /*event_handler=*/this);
        });
    fdf_testing_run_until_idle();
  ...
  async::WaitMethod<LoopbackTest, &LoopbackTest::OnChannelReady> loopback_chan_wait_{this};
};
...
TEST_F(LoopbackTest, SendManyCommandPackets) {
  ...
  for (uint8_t i = 0; i < 10; i++) {
    ...
    hci_client()->Send(request).Then(
        [&send_cb_count](fidl::Result<fhb::HciTransport::Send>& result) { send_cb_count++; });
  }
  fdf_testing_run_until_idle();
...
TEST_F(LoopbackTest, ReceiveManyEventPackets) {
  for (uint8_t i = 0; i < LoopbackDevice::kMaxReceiveUnackedPackets; i++) {
    std::array<uint8_t, 3> packet = {0x04,  // packet indicator (event),
                                     i, 0x05};
    zx_status_t status =
        chan().write(/*flags=*/0, packet.data(), static_cast<uint32_t>(packet.size()),
  ...
  ASSERT_EQ(received_packets().size(), LoopbackDevice::kMaxReceiveUnackedPackets);
  for (uint8_t i = 0; i < LoopbackDevice::kMaxReceiveUnackedPackets; i++) {
...
TEST_F(LoopbackTest, ReceiveAndQueueAndAckManyAclPackets) {
  ...
  // Ack 2x so that the 2 queued packets are sent.
  result = snoop_client()->AcknowledgePackets(fhb::SnoopAcknowledgePacketsRequest(2));
  ASSERT_TRUE(result.is_ok());
  fdf_testing_run_until_idle();
...
TEST_F(LoopbackTest, DropSnoopPackets) {
  ...
  const size_t num_packets_sent =
      LoopbackDevice::kMaxSnoopUnackedPackets + LoopbackDevice::kMaxSnoopQueueSize + num_dropped;
  ...
  fit::result<fidl::OneWayError> result = snoop_client()->AcknowledgePackets(
      fhb::SnoopAcknowledgePacketsRequest(LoopbackDevice::kMaxSnoopUnackedPackets));
  ASSERT_TRUE(result.is_ok());
  fdf_testing_run_until_idle();
  ...
  for (uint8_t i = LoopbackDevice::kMaxSnoopUnackedPackets;
       static_cast<size_t>(i) < snoop_packets().size(); i++) {
    auto& packet = snoop_packets()[i];
    EXPECT_EQ(packet.direction(), fhb::PacketDirection::kHostToController);
    EXPECT_EQ(packet.sequence(), i + 1 + num_dropped);
    ASSERT_EQ(packet.packet()->Which(), fhb::SnoopPacket::Tag::kAcl);
    std::vector<uint8_t> expected = {static_cast<uint8_t>(i + num_dropped), 0x07, 0x08};
    EXPECT_EQ(packet.packet()->acl().value(), expected);
  }
