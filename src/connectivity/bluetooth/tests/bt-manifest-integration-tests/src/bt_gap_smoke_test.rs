...
enum Event {
    ...
    Gatt2Server { _gatt2_server: Option<fbgatt2::Server_Proxy> },
...
async fn route_from_bt_gap_to_mock_client<S: DiscoverableProtocolMarker>(builder: &RealmBuilder) {
    let _ = builder
        ...
        .await
        .unwrap_or_else(|e| {
            panic!("failed routing {} from bt-gap to mock client: {:?}", S::PROTOCOL_NAME, e)
        });
}
...
async fn bt_gap_component_topology() {
    ...
    for event in vec![
        ...
        Event::Gatt2Server { _gatt2_server: None },
