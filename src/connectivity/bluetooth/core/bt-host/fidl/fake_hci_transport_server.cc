...
FakeHciTransportServer::FakeHciTransportServer(::fidl::ServerEnd<fhbt::HciTransport> server_end,
...
zx_status_t FakeHciTransportServer::SendEvent(const BufferView& event) {
  ::fit::result<::fidl::OneWayError> result = ::fidl::SendEvent(binding_)->OnReceive(
      ::fuchsia_hardware_bluetooth::ReceivedPacket::WithEvent(event.ToVector()));
  return result.is_ok() ? ZX_OK : result.error_value().status();
}

zx_status_t FakeHciTransportServer::SendAcl(const BufferView& buffer) {
  ::fit::result<::fidl::OneWayError> result = ::fidl::SendEvent(binding_)->OnReceive(
      ::fuchsia_hardware_bluetooth::ReceivedPacket::WithAcl(buffer.ToVector()));
  return result.is_ok() ? ZX_OK : result.error_value().status();
}
...
zx_status_t FakeHciTransportServer::SendIso(const BufferView& buffer) {
  ::fit::result<::fidl::OneWayError> result = ::fidl::SendEvent(binding_)->OnReceive(
      ::fuchsia_hardware_bluetooth::ReceivedPacket::WithIso(buffer.ToVector()));
  return result.is_ok() ? ZX_OK : result.error_value().status();
}
...
zx_status_t FakeHciTransportServer::ScoConnectionServer::Send(const BufferView& buffer) {
  ::fuchsia_hardware_bluetooth::ScoPacket packet(buffer.ToVector());
  fit::result<::fidl::OneWayError> result = ::fidl::SendEvent(binding_)->OnReceive(packet);
  return result.is_ok() ? ZX_OK : result.error_value().status();
}

void FakeHciTransportServer::ScoConnectionServer::Unbind() { binding_.Unbind(); }

void FakeHciTransportServer::ScoConnectionServer::Send(SendRequest& request,
                                                       SendCompleter::Sync& completer) {
  hci_server_->sco_packets_received_.emplace_back(BufferView(request.packet()));
  completer.Reply();
}

void FakeHciTransportServer::ScoConnectionServer::AckReceive(AckReceiveCompleter::Sync& completer) {
  hci_server_->sco_ack_receive_count_++;
}

void FakeHciTransportServer::ScoConnectionServer::Stop(StopCompleter::Sync& completer) {
...
void FakeHciTransportServer::Send(SendRequest& request, SendCompleter::Sync& completer) {
...
void FakeHciTransportServer::ConfigureSco(ConfigureScoRequest& request,
                                          ConfigureScoCompleter::Sync& completer) {
  if (!request.connection() || !request.coding_format() || !request.sample_rate() ||
      !request.encoding()) {
    return;
  }

  ASSERT_FALSE(sco_server_);
  sco_server_.emplace(std::move(request.connection().value()), dispatcher_, this);
