...
class ProfileServerTestScoConnected : public ProfileServerTestConnectedPeer {
 public:
  ...
  void SetUp(fidlbredr::ScoConnectionParameters conn_params) {
    ...
    std::optional<fidlbredr::ScoConnectionOnConnectionCompleteRequest> connection_complete;
...
TEST_F(ProfileServerTestConnectedPeer, ConnectL2capChannelParametersUseConnection) {
...
// Verifies that a BrEdrConnectionServer is correctly set up such that bytes written to the
// Connection are sent to the channel.
...
TEST_F(ProfileServerTestConnectedPeer, ConnectFailsDueToChannelActivationFailure) {
  server()->set_use_sockets(false);
...
// Verifies that a BrEdrConnectionServer is correctly set up such that bytes written to the
// Connection are sent to the channel.
TEST_F(ProfileServerTestConnectedPeer, ConnectionReceiverReturnsValidConnection) {
  server()->set_use_sockets(false);
...
TEST_F(ProfileServerTest, ConnectScoWithInvalidParameters) {
  ...
  sco_connection.set_error_handler([&](zx_status_t status) { sco_connection_status = status; });
  std::optional<fidlbredr::ScoConnectionOnConnectionCompleteRequest> connection_complete;
  ...
  EXPECT_EQ(connection_complete->error(), fidlbredr::ScoErrorCode::INVALID_ARGUMENTS);
...
TEST_F(ProfileServerTestConnectedPeer, ScoConnectionReadBeforeConnectionComplete) {
  std::vector<fidlbredr::ScoConnectionParameters> sco_params_list;
  sco_params_list.emplace_back(CreateScoConnectionParameters(fidlbredr::HfpParameterSet::D0));
  ...
  sco_connection->Read([](fidlbredr::ScoConnection_Read_Result result) { FAIL(); });
  RunLoopUntilIdle();
  ASSERT_TRUE(sco_connection_status);
  EXPECT_EQ(sco_connection_status.value(), ZX_ERR_IO_REFUSED);
}

TEST_F(ProfileServerTestConnectedPeer, ScoConnectionWriteBeforeConnectionComplete) {
  std::vector<fidlbredr::ScoConnectionParameters> sco_params_list;
  sco_params_list.emplace_back(CreateScoConnectionParameters(fidlbredr::HfpParameterSet::D0));
  ...
  sco_connection->Write(
      std::move(write),
      [](::fuchsia::bluetooth::bredr::ScoConnection_Write_Result result) { FAIL(); });
  RunLoopUntilIdle();
  ASSERT_TRUE(sco_connection_status);
  EXPECT_EQ(sco_connection_status.value(), ZX_ERR_IO_REFUSED);
}
...
TEST_P(AndroidSupportedFeaturesTest, AudioOffloadControllerStopSuccess) {
  ...
  auto callback = [&stop_callback_called](fidlbredr::AudioOffloadController_Stop_Result result) {
...
TEST_P(AndroidSupportedFeaturesTest, AudioOffloadControllerStopFail) {
  ...
  audio_offload_controller_client->Stop(
      [&cb_count](fidlbredr::AudioOffloadController_Stop_Result result) { cb_count++; });

  RunLoopUntilIdle();

  EXPECT_EQ(cb_count, 0u);

  // Verify that |audio_offload_controller_client| was closed with |ZX_ERR_UNAVAILABLE| epitaph
  ASSERT_TRUE(audio_offload_controller_epitaph.has_value());
  EXPECT_EQ(audio_offload_controller_epitaph.value(), ZX_ERR_UNAVAILABLE);
}

TEST_P(AndroidSupportedFeaturesTest, AudioOffloadControllerStopAfterAlreadyStopped) {
  const bool android_vendor_ext_support = GetParam().first;
  const uint32_t a2dp_offload_capabilities = GetParam().second;
...
TEST_F(ProfileServerTestScoConnected, ScoConnectionRead2Packets) {
  ...
  sco_connection()->Read([&](::fuchsia::bluetooth::bredr::ScoConnection_Read_Result result) {
    ASSERT_TRUE(result.is_response());
...
TEST_F(ProfileServerTestOffloadedScoConnected, ScoConnectionReadFails) {
  sco_connection()->Read(
      [&](::fuchsia::bluetooth::bredr::ScoConnection_Read_Result result) { FAIL(); });
  RunLoopUntilIdle();
  EXPECT_FALSE(sco_connection());
  ASSERT_TRUE(sco_conn_error());
...
TEST_F(ProfileServerTestScoConnected, ScoConnectionWriteTwice) {
  ...
  sco_connection()->Write(std::move(request_0), [&](fidlbredr::ScoConnection_Write_Result result) {
    ASSERT_TRUE(result.is_response());
    write_cb_0_count++;
  });
  RunLoopUntilIdle();
  ...
  sco_connection()->Write(std::move(request_1), [&](fidlbredr::ScoConnection_Write_Result result) {
    ASSERT_TRUE(result.is_response());
    write_cb_1_count++;
  });
  RunLoopUntilIdle();
...
TEST_F(ProfileServerTestScoConnected, ScoConnectionWriteMissingDataField) {
  ...
  sco_connection()->Write(std::move(request),
