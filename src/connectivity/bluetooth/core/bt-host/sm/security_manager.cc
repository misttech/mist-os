...
void SecurityManagerImpl::OnSecurityRequest(AuthReqField auth_req) {
  BT_ASSERT(!SecurityUpgradeInProgress());
...
void SecurityManagerImpl::OnFeatureExchange(PairingFeatures features,
                                            PairingRequestParams preq,
                                            PairingResponseParams pres) {
  BT_ASSERT(std::holds_alternative<std::unique_ptr<Phase1>>(current_phase_));
...
void SecurityManagerImpl::OnEncryptionChange(hci::Result<bool> enabled_result) {
  ...
  if (!SecurityUpgradeInProgress() || security_request_phase) {
    ...
    if (security_request_phase) {
      BT_ASSERT(role_ == Role::kResponder);
      BT_ASSERT(!request_queue_.empty());
...
void SecurityManagerImpl::OnPairingComplete(PairingData pairing_data) {
  ...
  if (features_->generate_ct_key.has_value()) {
    ...
    BT_ASSERT(pairing_data.peer_ltk == pairing_data.local_ltk);
...
void SecurityManagerImpl::OnPairingFailed(Error error) {
  ...
  bt_log(ERROR,
         ...
         phase_status.c_str());
  StopTimer();
  // TODO(https://fxbug.dev/42172514): implement "waiting interval" to prevent
  // repeated attempts as described in Vol 3, Part H, 2.3.6.
