...
void LegacyPairingState::InitiatePairing(StatusCallback status_cb) {
  ...
  if (state_ == State::kIdle) {
    ...
    // TODO(https://fxbug.dev/348676274): Do not downgrade to LP if peer was
    // previously bonded with SSP
    // TODO(https://fxbug.dev/348674937): Re-pair with SSP if peer was
  ...
  if (is_pairing()) {
    ...
    bt_log(
        INFO,
        "gap-bredr",
        "Already pairing on link %#.4x for peer id %s; blocking callback on completion",
        handle(),
        bt_str(peer_id_));
...
std::optional<hci_spec::LinkKey> LegacyPairingState::OnLinkKeyRequest() {
  ...
  // TODO(https://fxbug.dev/348676274): Do not downgrade to LP if peer was
  // previously bonded with SSP
  // TODO(https://fxbug.dev/348674937): Re-pair with SSP if peer was previously
  // bonded with LP
...
void LegacyPairingState::OnLinkKeyNotification(const UInt128& link_key,
  ...
  if (key_type != hci_spec::LinkKeyType::kCombination) {
    bt_log(
        WARN,
        "gap-bredr",
        "Legacy Pairing requires Combination key type but link %#.4x for peer id %s has type %s",
        handle(),
        bt_str(peer_id_),
...
void LegacyPairingState::OnAuthenticationComplete(
  ...
  if (is_pairing() && peer_->bredr() && peer_->bredr()->bonded() &&
    ...
    bt_log(
        INFO,
        "gap-bredr",
        "Re-initiating pairing on link %#.4x for peer id %s as remote device reports no key",
        handle(),
        bt_str(peer_id_));
...
void LegacyPairingState::OnEncryptionChange(hci::Result<bool> result) {
  ...
  if (state_ != State::kWaitEncryption) {
    ...
    bt_log(
        TRACE,
        "gap-bredr",
        "Ignoring encryption change event with result %s on link %#.4x for peer id %s in %s state",
        bt_str(result),
        handle(),
        bt_str(peer_id_),
        ToString(state_));
  ...
  if (encryption_status != pw::bluetooth::emboss::EncryptionStatus::
    ...
    bt_log(
        WARN,
        "gap-bredr",
        "E0 encryption must be used for legacy pairing when encryption is enabled");
    state_ = State::kFailed;
    SignalStatus(ToResult(HostError::kFailed), __func__);
    return;
  }
