...
void BrEdrConnectionManager::CompleteConnectionSetup(
  ...
  PairingStateType pairing_type = PairingStateType::kLegacyPairing;
  if (peer->IsSecureSimplePairingSupported()) {
    pairing_type = PairingStateType::kSecureSimplePairing;
  }
...
BrEdrConnectionManager::OnPinCodeRequest(const hci::EmbossEventPacket& event) {
  ...
  auto pin_code_cb = [this,
    ...
    if (pin) {
      // TODO(https://fxbug.dev/348700005): Support exponential backoff
