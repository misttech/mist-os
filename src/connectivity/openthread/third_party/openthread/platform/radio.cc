...
#include "spinel_driver_getter.h"
...
void platformRadioUpdateFdSet(otSysMainloopContext *aContext) {
  ...
  if (now < deadline) {
    ...
    if (remain < (static_cast<uint64_t>(aContext->m_timeout.tv_sec) * OT_US_PER_S +
                  static_cast<uint64_t>(aContext->m_timeout.tv_usec))) {
      aContext->m_timeout.tv_sec = static_cast<time_t>(remain / OT_US_PER_S);
      aContext->m_timeout.tv_usec = static_cast<suseconds_t>(remain % OT_US_PER_S);
    }
  } else {
    aContext->m_timeout.tv_sec = 0;
...
#if OPENTHREAD_POSIX_VIRTUAL_TIME
extern "C" void virtualTimeRadioProcess(otInstance *aInstance,
                                        const struct VirtualTimeEvent *aEvent) {
  OT_UNUSED_VARIABLE(aInstance);
  GetRadioSpinel().Process(aEvent);
}
#else
...
void otPlatDiagSetOutputCallback(otInstance *aInstance, otPlatDiagOutputCallback aCallback,
                                 void *aContext) {
  OT_UNUSED_VARIABLE(aInstance);
