        ...
        let res = if self.wrapping_key_id.lock().is_some() {
            if let Some(fscrypt_key) = self.get_fscrypt_key().await? {
                if !self.casefold() {
                    ...
                    let target_hash_code = fscrypt_key.hash_code(encrypted_name.as_bytes());
                    self.store()
                        .tree()
                    ...
                    loop {
                        match iter.get() {
                            ...
                            Some(ItemRef {
                                ...
                                && *casefold_hash == target_hash_code =>
                            {
    ...
    pub async fn create_child_dir(
        ...
        if self.wrapping_key_id.lock().is_some() {
            ...
            let casefold_hash = if self.casefold() {
                ...
                fscrypt_key.hash_code(encrypted_name.as_bytes())
            };
            transaction.add(
        ...
        if let Some(wrapping_key_id) = wrapping_key_id {
            if let Some(crypt) = self.store().crypt() {
                ...
                let casefold_hash = if self.casefold() {
                    ...
                    dir_key.hash_code(encrypted_name.as_bytes())
                };
        ...
        match (src_dir.wrapping_key_id(), dst.0.wrapping_key_id()) {
            (Some(src_id), Some(dst_id)) => {
                ...
                let src_casefold_hash = if src_dir.casefold() {
                    ...
                    key.hash_code(encrypted_src_name.as_bytes())
                };
                transaction.add(
                    store_id,
                    Mutation::replace_or_insert_object(
    ...
    if dst.0.wrapping_key_id().is_some() {
        if let Some(key) = dst.0.get_fscrypt_key().await? {
            ...
            let dst_casefold_hash = if dst.0.casefold() {
                ...
                key.hash_code(encrypted_dst_name.as_bytes())
            };
            transaction.add(
                store_id,
                Mutation::replace_or_insert_object(
...
mod tests {
    ...
    async fn test_create_casefold_encrypted_directory() {
        ...
        let object_id;
        {
            ...
            let casefold_hash = key.hash_code_casefold("bAr");
    ...
    fn find_out_of_order_sha256_long_prefix_pair(
        ...
        for i in 0..(1usize << 32) {
            ...
            let casefold_hash = key.hash_code_casefold(&filename);
    ...
    #[fuchsia::test]
    async fn test_casefold_hash_stability() {
        ...
        let mut object_id_to_casefold_hash = BTreeMap::new();
        loop {
            match iter.get() {
                Some(ItemRef {
                    key: ObjectKey { data: ObjectKeyData::EncryptedChild { casefold_hash, .. }, .. },
                    value:
                    ...
                    object_id_to_casefold_hash.insert(*object_id, *casefold_hash);
                }
                _ => {
                    break;
                }
        ...
        for &(name, casefolded_name, casefold_hash) in &CASEFOLD_NAMES {
            if let Some((object_id, _descriptor, _parent_dir_is_locked)) =
                directory.lookup(casefolded_name).await.expect("lookup")
            {
                let actual_hash = object_id_to_casefold_hash.get(&object_id);
                assert_eq!(
                    actual_hash,
                    Some(&casefold_hash),
