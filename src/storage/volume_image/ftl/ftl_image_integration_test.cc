...
void FillBlock(uint32_t block_number, size_t block_offset, std::span<uint8_t> block_view) {
...
TEST(FtlImageBootstrapTest, FtlDriverBootstrapsFromImageIsOk) {
  ...
  EXPECT_THAT(std::span<uint8_t>(page_buffer).subspan(0, 4096),
              testing::ElementsAreArray(std::span<uint8_t>(expected_page_buffer).subspan(0, 4096)));
  // Remainder of a mapping fitting on the same page, is filled with zeroes.
  EXPECT_THAT(std::span<uint8_t>(page_buffer).subspan(4096, 4096), testing::Each(testing::Eq(0)));
