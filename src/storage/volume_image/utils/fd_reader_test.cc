...
void Write(int fd, std::span<const char> buffer) {
...
TEST(FdReaderTest, ReadReturnsCorrectContents) {
  ...
  ASSERT_NO_FATAL_FAILURE(
      Write(target_fd.get(), std::span(kFileContents.data(), kFileContents.size())));
...
TEST(FdReaderTest, ReadAreIdempotent) {
  ...
  for (uint64_t offset = 0; offset < kFileContents.size() - 1; ++offset) {
    auto read_result = reader.Read(offset, std::span(buffer.data(), buffer.size() - offset));
    ASSERT_TRUE(read_result.is_ok()) << read_result.error();
...
TEST(FdReaderTest, ReadOutOfBoundsIsError) {
  ...
  EXPECT_TRUE(reader.Read(kFileContents.size(), std::span(buffer.data(), 1)).is_error());

  // Try to read too much.
  EXPECT_TRUE(reader.Read(1, std::span(buffer.data(), buffer.size())).is_error());
}
