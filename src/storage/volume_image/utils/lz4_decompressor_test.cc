...
TEST(Lz4DecompressorTest, PrepareAfterConstructionIsOk) {
  ...
  EXPECT_TRUE(
      decompressor.Prepare([](std::span<const uint8_t> buffer) { return fpromise::ok(); }).is_ok());
}
...
TEST(Lz4DecompressorTest, DecompressWithoutPrepareIsError) {
  ...
  EXPECT_TRUE(decompressor.Decompress(std::span<const uint8_t>()).is_error());
}
...
TEST(Lz4DecompressorTest, DecompressWithPrepareAndSizeHintIsOk) {
  ...
  ASSERT_TRUE(decompressor
                  .Prepare([](std::span<const uint8_t> buffer) {
...
TEST(Lz4DecompressorTest, DecompressOnMultipleStepsIsOk) {
  ...
  ASSERT_TRUE(decompressor
                  .Prepare([&](std::span<const uint8_t> buffer) {
                    EXPECT_THAT(buffer,
                                testing::ElementsAreArray(std::span<const uint8_t>(kData).subspan(
  ...
  while (!is_decompression_done) {
    auto decompression_result =
        decompressor.Decompress(std::span<uint8_t>(data_or.value()).subspan(total_consumed_bytes));
