...
void Read(int fd, std::span<char> buffer) {
...
TEST(FdWriterTest, WriteUpdateContentsReturnsNoError) {
  ...
  auto write_result = writer.Write(
      0, std::span(reinterpret_cast<const uint8_t*>(kFileContents.data()), kFileContents.size()));
...
TEST(FdWriterTest, WriteReturnsCorrectContentsAtOffset) {
  ...
  auto write_result = writer.Write(
      kOffset,
      std::span(reinterpret_cast<const uint8_t*>(kFileContents.data()), kFileContents.size())
          .subspan(kOffset));
  ASSERT_TRUE(write_result.is_ok()) << write_result.error();
...
TEST(FdWriterTest, WritesAreIdempotent) {
  ...
  for (unsigned int i = 0; i < kFileContents.size() - 1; i++) {
    auto write_result = writer.Write(
        i, std::span(reinterpret_cast<const uint8_t*>(kFileContents.data()), kFileContents.size())
               .subspan(i));
    ASSERT_TRUE(write_result.is_ok()) << write_result.error();
  }
...
TEST(FdWriterTest, WritingPastEndOfFileIsOk) {
  ...
  EXPECT_TRUE(writer
                  .Write(0, std::span(reinterpret_cast<const uint8_t*>(kFileContents.data()),
                                      kFileContents.size()))
                  .is_ok());
  ...
  EXPECT_TRUE(writer
                  .Write(kFileContents.size(),
                         std::span(reinterpret_cast<const uint8_t*>(kFileContents.data()),
                                   kFileContents.size()))
                  .is_ok());
