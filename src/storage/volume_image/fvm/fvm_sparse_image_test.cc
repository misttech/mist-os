...
class FakeReader : public Reader {
 public:
  explicit FakeReader(
      fit::function<fpromise::result<void, std::string>(uint64_t, std::span<uint8_t>)> filler)
 ...
 private:
  fit::function<fpromise::result<void, std::string>(uint64_t offset, std::span<uint8_t>)> filler_;
};

// Fake writer implementations that writes into a provided buffer.
class BufferWriter : public Writer {
 public:
  explicit BufferWriter(std::span<uint8_t> buffer) : buffer_(buffer) {}
...
template <int shift>
fpromise::result<void, std::string> GetContents(uint64_t offset, std::span<uint8_t> buffer) {
...
class SerializedImageContainer {
 public:
  ...
  std::span<uint8_t> AsSpan() {
    return std::span<uint8_t>(reinterpret_cast<uint8_t*>(serialized_image_.get()),
  ...
  std::vector<std::span<const uint8_t>> PartitionExtents(size_t index) {
    auto view = std::span(serialized_image_->extent_data);
    if (index == 0) {
...
class ErrorWriter final : public Writer {
 public:
  ...
  fpromise::result<void, std::string> Write(
      [[maybe_unused]] uint64_t offset, [[maybe_unused]] std::span<const uint8_t> buffer) final {
...
TEST(FvmSparseWriteImageTest, WithReadErrorIsError) {
  ...
  auto partition_1_result = Partition::Create(
      ...
      std::make_unique<FakeReader>(
          []([[maybe_unused]] uint64_t offset, [[maybe_unused]] std::span<uint8_t> buffer) {
...
class BufferReader final : public Reader {
 ...
 private:
  ...
  std::span<const uint8_t> image_buffer_;
...
class FvmSparseReaderImpl final : public fvm::ReaderInterface {
 public:
  explicit FvmSparseReaderImpl(std::span<const uint8_t> buffer) : buffer_(buffer) {}
...
TEST(FvmSparseWriteImageTest, WrittenImageIsCompatibleWithLegacyImplementation) {
  ...
  // Partition 1 metadata.
  {
    ...
    const auto partition_extent_descriptors = std::span<const fvm::ExtentDescriptor>(
  ...
  for (const auto& partition : descriptor.partitions()) {
    for (const auto& mapping : partition.address().mappings) {
      ...
      auto read_result = partition.reader()->Read(
          mapping.source, std::span(original_content.data(), original_content.size()));
...
TEST(ConvertToFvmMetadataTest, WithNoPartitionsIsOk) {
  ...
  auto metadata_or = fvm_sparse_internal::ConvertToFvmMetadata(
      header, std::span<fvm_sparse_internal::PartitionEntry>());
...
TEST(ConvertToFvmMetadataTest, WithSinglePartitionsAndNoSlicesIsOk) {
  ...
  auto metadata_or = fvm_sparse_internal::ConvertToFvmMetadata(
      header, std::span<fvm_sparse_internal::PartitionEntry>(&entry, 1));
  ASSERT_TRUE(metadata_or.is_ok()) << metadata_or.error();
  auto metadata = metadata_or.take_value();
  ...
  for (auto i = 1u; i < header.GetPartitionTableEntryCount(); ++i) {
    ...
    for (auto& b : std::span<uint8_t>(entry.type).subspan(1)) {
      EXPECT_EQ(b, 0);
    }
...
TEST(ConvertToFvmMetadataTest, WithsMultiplePartitionsAndSlicesIsOk) {
  ...
  for (auto i = 1u; i < header.GetPartitionTableEntryCount(); ++i) {
    ...
    for (auto& b : std::span<uint8_t>(actual_entry.type).subspan(1)) {
      EXPECT_EQ(b, 0);
    }
