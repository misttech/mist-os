...
impl StorageHostService {
    ...
    async fn start(self: &Arc<Self>, device: fblock::BlockProxy) -> Result<(), zx::Status> {
        ...
        // XXX bug number
        // TODO(jfsulliv): It would be better if `start` failed on a malformed device, rather than
        // hiding this state from fshost.  However, fs_management isn't well set up to deal with
        // this, because it ties the outgoing directory to a successful return from `start` (see
        // `ServingMultiVolumeFilesystem`), and fshost resolves the queued requests for the
        // filesystem only after `start` is successful.  We should refactor fs_management and fshost
        // to better support this case, which might require changing how queueing works so there's
        // more flexibility to either resolve queueing when the component starts up (what we need
        // here), or when `Start` is successful (what a filesystem like Fxfs needs).
