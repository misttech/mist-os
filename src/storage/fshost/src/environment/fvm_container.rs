...
pub struct FvmContainer(ServingMultiVolumeFilesystem);
        ...
        if let Some(data_label) = check_volumes(self.get_volumes().await?) {
            ...
            match self
                ...
                .open_volume(
                    &data_label,
                    MountOptions {
                        uri: Some(format!("#meta/{format}.cm")),
    ...
    async fn format_data(&mut self, launcher: &FilesystemLauncher) -> Result<Filesystem, Error> {
        ...
        self.0
            .create_volume(
                ...
                MountOptions {
                    uri: Some(format!("#meta/{}.cm", launcher.config.data_filesystem_format)),
                    ..Default::default()
                },
            )
            .await
            .map(|_| Filesystem::ServingVolumeInMultiVolume(None, DATA_PARTITION_LABEL.to_string()))
    }
