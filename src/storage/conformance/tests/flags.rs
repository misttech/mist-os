...
#[fuchsia::test]
async fn file_set_flags2_unsupported_flags() {
    let harness = TestHarness::new().await;
    if !harness.config.supports_get_set_flags2 {
        return;
    }
    ...
    // Should not be able to set other flags
    let err = file
        .set_flags2(fio::Flags::PERM_GET_ATTRIBUTES)
        .await
        .expect("set_flags2 failed")
...
async fn file_set_flags2_invalid_flags() {
    ...
    // Although we can set Flags.FILE_APPEND, we cannot set other flags
    let err = file
...
async fn node_reference_set_flags2_not_supported() {
    ...
    let node_reference = dir
        .open3_node::<fio::NodeMarker>(&TEST_FILE, fio::Flags::empty(), None)
        .await
        .expect("open3 failed");
