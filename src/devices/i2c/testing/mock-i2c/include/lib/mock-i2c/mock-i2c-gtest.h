...
#ifndef SRC_DEVICES_I2C_TESTING_MOCK_I2C_INCLUDE_LIB_MOCK_I2C_MOCK_I2C_GTEST_H_
#define SRC_DEVICES_I2C_TESTING_MOCK_I2C_INCLUDE_LIB_MOCK_I2C_MOCK_I2C_GTEST_H_
...
class MockI2cGtest : public fidl::WireServer<fuchsia_hardware_i2c::Device> {
 public:
  // Checks that the next operation is a read and returns the contents of buf.
  MockI2cGtest& ExpectRead(std::vector<uint8_t> buf, zx_status_t status = ZX_OK) {
  ...
  MockI2cGtest& ExpectReadStop(std::vector<uint8_t> buf, zx_status_t status = ZX_OK) {
  ...
  MockI2cGtest& ExpectWrite(std::vector<uint8_t> buf, zx_status_t status = ZX_OK) {
  ...
  // contents of buf.
  MockI2cGtest& ExpectWriteStop(std::vector<uint8_t> buf, zx_status_t status = ZX_OK) {
  ...
  void Transfer(TransferRequestView request, TransferCompleter::Sync& completer) override {
    fidl::Arena arena;
    std::vector<fidl::VectorView<uint8_t>> read_ops;
 ...
 private:
  ...
  void CheckI2cOp(const fuchsia_hardware_i2c::wire::Transaction& op,
    ...
    ASSERT_FALSE(expectations_.empty()) << "No more transactions are expected";
    ...
    if (!is_read) {
      EXPECT_THAT(exp.data,
                  ::testing::ElementsAreArray(cpp20::span(op.data_transfer().write_data())))
          << "Transaction data mismatch";
    }
  }

  std::vector<I2cExpectation> expectations_;
  size_t expectations_index_ = 0;
};

}  // namespace mock_i2c

#endif  // SRC_DEVICES_I2C_TESTING_MOCK_I2C_INCLUDE_LIB_MOCK_I2C_MOCK_I2C_GTEST_H_
