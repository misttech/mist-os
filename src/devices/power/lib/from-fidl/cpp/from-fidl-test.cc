...
// Verify that `CreateTransition()` can convert a fuchsia_hardware_power::wire::Transition value
// into a fdf_power::Transition value.
TEST(FromFidlTest, TransitionFromWire) {
  fidl::Arena arena;
...
// Verify that `CreateLevelTuple()` can convert a fuchsia_hardware_power::wire::LevelTuple value
// into a fdf_power::LevelTuple value.
TEST(FromFidlTest, LevelTupleFromWire) {
  fidl::Arena arena;
...
// Verify `CreateLevelTuple()` will fail if child level is missing.
TEST(FromFidlTest, LevelTupleFromWireMissingChildLevel) {
  fidl::Arena arena;
...
// Verify `CreateLevelTuple()` will fail if parent level is missing.
TEST(FromFidlTest, LevelTupleFromWireMissingParentLevel) {
  fidl::Arena arena;
...
// Verify that `CreatePowerLevel()` can convert a fuchsia_hardware_power::wire::PowerLevel value
// into a fdf_power::PowerLevel value.
TEST(FromFidlTest, PowerLevelFromWire) {
  fidl::Arena arena;
...
// Verify `CreatePowerLevel()` will fail if level is missing.
TEST(FromFidlTest, PowerLevelFromWireMissingLevel) {
  fidl::Arena arena;
  std::vector<fuchsia_hardware_power::wire::Transition> transitions;
  auto fidl =
      fuchsia_hardware_power::wire::PowerLevel::Builder(arena)
...
// Verify `CreatePowerLevel()` will fail if name is missing.
TEST(FromFidlTest, PowerLevelFromWireMissingName) {
  fidl::Arena arena;
  std::vector<fuchsia_hardware_power::wire::Transition> transitions;
  auto fidl =
      fuchsia_hardware_power::wire::PowerLevel::Builder(arena)
...
// Verify `CreatePowerLevel()` will succeed if transitions are missing.
TEST(FromFidlTest, PowerLevelFromWireMissingTransitions) {
  fidl::Arena arena;
...
// Verify that `CreatePowerElement()` can convert a fuchsia_hardware_power::wire::PowerElement value
// into a fdf_power::PowerElement value.
TEST(FromFidlTest, PowerElementFromWire) {
  fidl::Arena arena;
...
// Verify `CreatePowerElement()` will fail if name is missing.
TEST(FromFidlTest, PowerElementFromWireMissingName) {
  fidl::Arena arena;
  std::vector<fuchsia_hardware_power::wire::PowerLevel> levels;
  auto fidl =
      fuchsia_hardware_power::wire::PowerElement::Builder(arena)
...
// Verify `CreatePowerElement()` will succeed if levels are missing.
TEST(FromFidlTest, PowerElementFromWireMissingLevels) {
  fidl::Arena arena;
  auto fidl =
...
// Verify that `CreateParentElement()` can convert a fuchsia_hardware_power::wire::ParentElement
// value with type "sag" into a fdf_power::ParentElement value.
TEST(FromFidlTest, ParentElementFromWireWithSag) {
...
// Verify that `CreateParentElement()` can convert a fuchsia_hardware_power::wire::ParentElement
// value with type "instance name" into a fdf_power::ParentElement value.
TEST(FromFidlTest, ParentElementFromWireWithInstanceName) {
  fidl::Arena arena;
  auto fidl =
...
// Verify that `CreatePowerDependency()` can convert a fuchsia_hardware_power::wire::PowerDependency
// value into a fdf_power::PowerDependency value.
TEST(FromFidlTest, PowerDependencyFromWire) {
  fidl::Arena arena;
...
// Verify `CreatePowerDependency()` will fail if child is missing.
TEST(FromFidlTest, PowerDependencyFromWireMissingChild) {
  fidl::Arena arena;
  std::vector<fuchsia_hardware_power::wire::LevelTuple> level_deps;
  auto fidl =
      fuchsia_hardware_power::wire::PowerDependency::Builder(arena)
...
// Verify `CreatePowerDependency()` will fail if parent is missing.
TEST(FromFidlTest, PowerDependencyFromWireMissingParent) {
  fidl::Arena arena;
  std::vector<fuchsia_hardware_power::wire::LevelTuple> level_deps;
  auto fidl =
      fuchsia_hardware_power::wire::PowerDependency::Builder(arena)
...
// Verify `CreatePowerDependency()` will succeed if level dependencies are missing.
TEST(FromFidlTest, PowerDependencyFromWireMissingLevelDeps) {
  fidl::Arena arena;
...
// Verify `CreatePowerDependency()` will fail if strength is missing.
TEST(FromFidlTest, PowerDependencyFromWireMissingStrength) {
  fidl::Arena arena;
  std::vector<fuchsia_hardware_power::wire::LevelTuple> level_deps;
  auto fidl =
      fuchsia_hardware_power::wire::PowerDependency::Builder(arena)
...
// Verify that `CreatePowerElementConfiguration()` can convert a
// fuchsia_hardware_power::wire::PowerElementConfiguration value into a
// fdf_power::PowerElementConfiguration value.
TEST(FromFidlTest, PowerElementConfigurationFromWire) {
  fidl::Arena arena;
  auto element =
      fuchsia_hardware_power::wire::PowerElement::Builder(arena).name("test power element").Build();
...
// Verify `CreatePowerElementConfiguration()` will fail if element is missing.
TEST(FromFidlTest, PowerElementConfigurationFromWireMissingElement) {
  fidl::Arena arena;
  std::vector<fuchsia_hardware_power::wire::PowerDependency> dependencies;
  auto fidl = fuchsia_hardware_power::wire::PowerElementConfiguration::Builder(arena)
                  .dependencies(
...
// Verify `CreatePowerElementConfiguration()` will succeed if dependencies are
// missing.
...
TEST(FromFidlTest, PowerElementConfigurationFromWireMissingDependencies) {
  fidl::Arena arena;
  auto element =
      fuchsia_hardware_power::wire::PowerElement::Builder(arena).name("test power element").Build();
  auto fidl = fuchsia_hardware_power::wire::PowerElementConfiguration::Builder(arena)
