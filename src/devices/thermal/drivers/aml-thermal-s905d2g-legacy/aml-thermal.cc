...
zx_status_t AmlThermal::Create(void* ctx, zx_device_t* device) {
  ...
  if (smc_resource_result.is_error()) {
    zxlogf(ERROR, "Failed to get smc: %s", smc_resource_result.status_string());
    return smc_resource_result.status_value();
  }
  zx::resource smc_resource = std::move(smc_resource_result.value());
