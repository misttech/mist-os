...
class SpmiCtlTest : public zxtest::Test {
 public:
  void SetUp() override {
    ...
    spmi_ = std::make_unique<FakeSpmi>();
  }
  ...
  int CallSpmiCtl(std::vector<std::string> args) {
    ...
    fidl::ClientEnd<fuchsia_hardware_spmi::Device> client;
...
TEST_F(SpmiCtlTest, UnknownCommands) {
  EXPECT_EQ(CallSpmiCtl({"spmi-ctl", "-b"}), -1);
  EXPECT_EQ(CallSpmiCtl({"spmi-ctl", "--bad"}), -1);
}

TEST_F(SpmiCtlTest, ReadWrite) {
  // Successful.
  std::vector<uint8_t> kCannedData = {0x12, 0x34, 0x56};
  ASSERT_EQ(CallSpmiCtl({"spmi-ctl", "-a", "0x1234", "-w", "0x12", "0x34", "0x56"}), 0);
  EXPECT_TRUE(spmi_->data() == kCannedData);
  EXPECT_EQ(spmi_->address(), 0x1234);
  ASSERT_EQ(CallSpmiCtl({"spmi-ctl", "-a", "0x1234", "-r", "4"}), 0);
  EXPECT_EQ(spmi_->read_size(), 4);
  EXPECT_EQ(spmi_->address(), 0x1234);

  // Errors.
  ASSERT_EQ(CallSpmiCtl({"spmi-ctl", "-w", "0x12", "0x34", "0x56"}), -1);  // No address.
  ASSERT_EQ(CallSpmiCtl({"spmi-ctl", "-a", "0x5678", "-r", "4"}), -1);     // Unknown address.
  ASSERT_EQ(CallSpmiCtl({"spmi-ctl", "-a", "0x10000", "-r", "4"}), -1);    // Address too big.
  ASSERT_EQ(CallSpmiCtl({"spmi-ctl", "-a", "0x1234", "-r", "9"}), -1);     // Read too big.
  ASSERT_EQ(CallSpmiCtl({"spmi-ctl", "-a", "0x1234", "-w"}), -1);          // Write no data.
  ASSERT_EQ(
      CallSpmiCtl({"spmi-ctl", "-a", "0x1234", "-w", "1", "2", "3", "4", "5", "6", "7", "8", "9"}),
      -1);  // Write too big.
}
