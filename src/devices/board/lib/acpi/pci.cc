...
static acpi::status<> resource_report_callback(ACPI_RESOURCE* res, ResourceContext* ctx) {
  ...
  if (resource_is_memory(res)) {
    ...
    if (!addr.min_address_fixed || !addr.max_address_fixed || addr.maximum < addr.minimum) {
      LTRACEF("ACPI found bad _CRS address entry\n");
      return acpi::ok();
    }
    ...
    if (io.minimum != io.maximum) {
      LTRACEF("ACPI found bad _CRS IO entry\n");
      return acpi::ok();
    }
  ...
  LTRACEF("ACPI range modification: %sing %s %016lx %016lx\n", add_range ? "add" : "subtract",
  ...
  if (status != ZX_OK) {
    if (add_range) {
      LTRACEF("Failed to add range: [%#lx - %#lx] (%#lx): %d\n", base, base + len, len, status);
    } else {
      // If we are subtracting a range and fail, abort.  This is bad.
      LTRACEF("Failed to subtract range [%#lx - %#lx] (%#lx): %d\n", base, base + len, len, status);
      return acpi::error(AE_ERROR);
    }
  }
  return acpi::ok();
...
static acpi::status<> walk_devices_callback(ACPI_HANDLE object, ResourceContext* ctx,
  ...
  if (res.is_error()) {
    LTRACEF("acpi::GetObjectInfo failed %d\n", res.error_value());
 ...
 */
zx_status_t scan_acpi_tree_for_resources(acpi::Acpi* acpi, void* mmio_resource_handle) {
...
static zx_status_t read_mcfg_table(fbl::Vector<McfgAllocation>* mcfg_table) {
  ...
  if (table_bytes % sizeof(McfgAllocation)) {
    LTRACEF("MCFG table has invalid size %zu\n", table_bytes);
    return ZX_ERR_INTERNAL;
  }
  ...
  for (unsigned i = 0; i < table_bytes / sizeof(acpi_mcfg_allocation); i++) {
    ...
    LTRACEF("MCFG allocation %u (Addr = %#llx, Segment = %u, Start = %u, End = %u)\n", i,
            entry->Address, entry->PciSegment, entry->StartBusNumber, entry->EndBusNumber);
    mcfg_table->push_back({.address = entry->Address,
                           .pci_segment = entry->PciSegment,
                           .start_bus_number = entry->StartBusNumber,
                           .end_bus_number = entry->EndBusNumber},
                          &ac);
    if (!ac.check()) {
...
zx_status_t pci_init_interrupts(zx_device_t* parent, acpi::Acpi* acpi, ACPI_HANDLE object,
  ...
  if (acpi::GetPciRootIrqRouting(acpi, object, dev_ctx) != AE_OK) {
    LTRACEF("Failed to obtain PCI IRQ routing information, legacy IRQs will not function\n");
  }

  fbl::AllocChecker ac;
  fbl::Array<pci_legacy_irq> irq_list(new (&ac) pci_legacy_irq[dev_ctx->irqs.size()]{},
                                      dev_ctx->irqs.size());
  if (!ac.check()) {
    return ZX_ERR_NO_MEMORY;
  }
  ...
  for (const auto& e : dev_ctx->irqs) {
    ...
    if (status != ZX_OK) {
      LTRACEF("Couldn't create resource for legacy vector %#x: %s, skipping it\n", vector,
    ...
    if (status != ZX_OK) {
      LTRACEF("Couldn't create irq for legacy vector %#x: %s, skipping it\n", vector,
              zx_status_get_string(status));
      continue;
    }
...
zx_status_t pci_init_segment_and_ecam(zx_device_t* parent, acpi::Acpi* acpi, ACPI_HANDLE object,
  ...
  if (bbn.is_error() && acpi_to_zx_status(bbn.error_value()) != ZX_ERR_NOT_FOUND) {
    LTRACEF("Unable to read _BBN for '%s' (%d), assuming base bus of 0\n", dev_ctx->name,
            bbn.error_value());
  ...
  if (seg.is_error()) {
    ...
    LTRACEF("Unable to read _SEG for '%s' (%d), assuming segment group 0.\n", dev_ctx->name,
            seg.error_value());
  } else {
  ...
  if (status == ZX_OK) {
    ...
    if (found_bbn && mcfg_alloc.start_bus_number != pinfo.start_bus_num) {
      LTRACEF("conflicting base bus num for '%s', _BBN reports %u and MCFG reports %u\n",
    ...
    if (pinfo.segment_group != 0 && pinfo.segment_group != mcfg_alloc.pci_segment) {
      LTRACEF("conflicting segment group for '%s', _BBN reports %u and MCFG reports %u\n",
    ...
    // status = zx_vmo_create_physical(get_mmio_resource(parent), vmo_base, ecam_size,
    // &pinfo.cam.vmo);
    // zx_vmo_create_physical
    {
      //  create a vm object
      fbl::RefPtr<VmObjectPhysical> vmo;
      status = VmObjectPhysical::Create(vmo_base, ecam_size, &vmo);
      if (status != ZX_OK) {
        return status;
      }
      ...
      zx_rights_t rights;
      status = VmObjectDispatcher::Create(ktl::move(vmo), ecam_size,
                                          VmObjectDispatcher::InitialMutability::kMutable,
                                          &kernel_handle, &rights);
      if (status != ZX_OK) {
        LTRACEF("couldn't create VMO for ecam, mmio cfg will not work: %s!\n",
                zx_status_get_string(status));
        return status;
      }

      auto vmo_dispatcher = kernel_handle.release();
      pinfo.cam.vmo_list = (uint8_t*)fbl::ExportToRawPtr(&vmo_dispatcher);
      pinfo.cam.vmo_count = 1;
    }
  ...
  if (LOCAL_TRACE >= 2) {
    char log[128];
    int len = snprintf(log, sizeof(log), "%s { acpi_obj(%p), bus range: %u:%u, segment: %u",
                       dev_ctx->name, dev_ctx->acpi_object, pinfo.start_bus_num, pinfo.end_bus_num,
                       pinfo.segment_group);
    if (pinfo.cam.vmo_count > 0) {
      len += snprintf(log + len, sizeof(log) - len, ", ecam base: %#" PRIxPTR, mcfg_alloc.address);
    }
    len += snprintf(log + len, sizeof(log) - len, " }");
    log[len] = '\0';
    LTRACEF("%s\n", log);
  }
...
zx_status_t pci_root_host_init(zx_device_t* parent, acpi::Acpi* acpi) {
  ...
  if (st != ZX_OK) {
    LTRACEF("Couldn't read MCFG table, PCI config MMIO will be unavailable: %s\n",
            zx_status_get_string(st));
  }

  st = scan_acpi_tree_for_resources(acpi, get_mmio_resource(parent));
  if (st != ZX_OK) {
    LTRACEF("Scanning acpi resources failed: %s\n", zx_status_get_string(st));
    return st;
  }
...
zx_status_t pci_init(zx_device_t* parent, ACPI_HANDLE object,
  ...
  if (status != ZX_OK) {
    LTRACEF("Error initializing PCI root host: %s\n", zx_status_get_string(status));
    return status;
  }
  ...
  if (status != ZX_OK) {
    LTRACEF("Initializing %s ecam and bus information failed: %s\n", dev_ctx.name,
            zx_status_get_string(status));
    return status;
  }
  ...
  if (status != ZX_OK) {
    LTRACEF("Initializing %s interrupt information failed: %s\n", dev_ctx.name,
            zx_status_get_string(status));
    return status;
  }
  ...
  if (status != ZX_OK) {
    LTRACEF("failed to add pciroot device for '%s': %d\n", name, status);
  } else {
    LTRACEF("published pciroot '%s'\n", name);
  }
