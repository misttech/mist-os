...
TEST_F(DisplayCompositorTest, DISABLED_HardwareFrameCorrectnessTest) {
  ...
  DisplayInfo display_info = {resolution, {kPixelFormat}};
  scenic_impl::display::Display display({kDisplayId.value}, resolution.x, resolution.y);
...
// constraints, and `flatland::DisplayCompositor` has been modified to reflect this.  Note: there
// will likely be changes needed to the expected call sequence in the mock display coordinator.
TEST_F(DisplayCompositorTest, DISABLED_HardwareFrameCorrectnessWith90DegreeRotationTest) {
...
// constraints, and `flatland::DisplayCompositor` has been modified to reflect this.  Note: there
// will likely be changes needed to the expected call sequence in the mock display coordinator.
TEST_F(DisplayCompositorTest, DISABLED_HardwareFrameCorrectnessWith180DegreeRotationTest) {
...
// constraints, and `flatland::DisplayCompositor` has been modified to reflect this.  Note: there
// will likely be changes needed to the expected call sequence in the mock display coordinator.
TEST_F(DisplayCompositorTest, DISABLED_HardwareFrameCorrectnessWith270DegreeRotationTest) {
...
// constraints, and `flatland::DisplayCompositor` has been modified to reflect this.  Note: there
// will likely be changes needed to the expected call sequence in the mock display coordinator.
TEST_F(DisplayCompositorTest, DISABLED_HardwareFrameCorrectnessWithLeftRightFlipTest) {
...
// constraints, and `flatland::DisplayCompositor` has been modified to reflect this.  Note: there
// will likely be changes needed to the expected call sequence in the mock display coordinator.
TEST_F(DisplayCompositorTest, DISABLED_HardwareFrameCorrectnessWithUpDownFlipTest) {
  glm::mat3 matrix = glm::scale(glm::mat3(), glm::vec2(10, 20));
...
// will likely be changes needed to the expected call sequence in the mock display coordinator.
TEST_F(DisplayCompositorTest,
       DISABLED_HardwareFrameCorrectnessWithLeftRightFlip90DegreeRotationTest) {
  // After scale and 90 CCW rotation, the new top-left corner would be (0, -10). Translate
  // back to position.
  glm::mat3 matrix = glm::translate(glm::mat3(1.0), glm::vec2(0, 10));
  matrix = glm::rotate(matrix, -glm::half_pi<float>());
...
// constraints, and `flatland::DisplayCompositor` has been modified to reflect this.  Note: there
// will likely be changes needed to the expected call sequence in the mock display coordinator.
TEST_F(DisplayCompositorTest, DISABLED_HardwareFrameCorrectnessWithUpDownFlip90DegreeRotationTest) {
  // After scale and 90 CCW rotation, the new top-left corner would be (0, -10). Translate
  // back to position.
  glm::mat3 matrix = glm::translate(glm::mat3(1.0), glm::vec2(0, 10));
  matrix = glm::rotate(matrix, -glm::half_pi<float>());
...
// will likely be changes needed to the expected call sequence in the mock display coordinator.
TEST_F(DisplayCompositorTest,
       DISABLED_HardwareFrameCorrectnessWithLeftRightFlip180DegreeRotationTest) {
  // After scale and 180 CCW rotation, the new top-left corner would be (-10, -20).
  // Translate back to position.
  glm::mat3 matrix = glm::translate(glm::mat3(1.0), glm::vec2(10, 20));
  matrix = glm::rotate(matrix, -glm::pi<float>());
...
// will likely be changes needed to the expected call sequence in the mock display coordinator.
TEST_F(DisplayCompositorTest,
       DISABLED_HardwareFrameCorrectnessWithUpDownFlip180DegreeRotationTest) {
  // After scale and 180 CCW rotation, the new top-left corner would be (-10, -20).
  // Translate back to position.
  glm::mat3 matrix = glm::translate(glm::mat3(1.0), glm::vec2(10, 20));
  matrix = glm::rotate(matrix, -glm::pi<float>());
...
// will likely be changes needed to the expected call sequence in the mock display coordinator.
TEST_F(DisplayCompositorTest,
       DISABLED_HardwareFrameCorrectnessWithLeftRightFlip270DegreeRotationTest) {
  // After scale and 270 CCW rotation, the new top-left corner would be (-20, 0). Translate
  // back to position.
  glm::mat3 matrix = glm::translate(glm::mat3(1.0), glm::vec2(20, 0));
  matrix = glm::rotate(matrix, -glm::three_over_two_pi<float>());
...
// will likely be changes needed to the expected call sequence in the mock display coordinator.
TEST_F(DisplayCompositorTest,
       DISABLED_HardwareFrameCorrectnessWithUpDownFlip270DegreeRotationTest) {
  // After scale and 270 CCW rotation, the new top-left corner would be (-20, 0). Translate
  // back to position.
  glm::mat3 matrix = glm::translate(glm::mat3(1.0), glm::vec2(20, 0));
  matrix = glm::rotate(matrix, -glm::three_over_two_pi<float>());
...
// constraints, and `flatland::DisplayCompositor` has been modified to reflect this.  Note: there
// will likely be changes needed to the expected call sequence in the mock display coordinator.
TEST_F(DisplayCompositorTest, DISABLED_ChecksDisplayImageSignalFences) {
  ...
  scenic_impl::display::Display display({kDisplayId.value}, kResolution.x, kResolution.y);
  display_compositor_->AddDisplay(&display, display_info, /*num_vmos*/ 0,
                                  /*out_buffer_collection*/ nullptr);

  // Set expectation for rendering image on layer.
  std::vector<fuchsia_hardware_display::wire::LayerId> active_layers = {{.value = 1}};
  zx::event imported_event;
  EXPECT_CALL(*mock_display_coordinator_, ImportEvent(_, _))
      .Times(1)
      .WillOnce(testing::Invoke(
          [&imported_event](fuchsia_hardware_display::wire::CoordinatorImportEventRequest* request,
