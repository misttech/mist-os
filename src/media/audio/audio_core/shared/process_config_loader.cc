...
#include "src/media/audio/audio_core/shared/schema/audio_core_config_schema.inl"
...
PipelineConfig::MixGroup ParseMixGroupFromJsonObject(const rapidjson::Value& value) {
  ...
  if (it != value.MemberEnd()) {
    FX_CHECK(it->value.IsUint());
    FX_CHECK(it->value.GetUint() <= std::numeric_limits<int16_t>::max());
    mix_group.output_channels = static_cast<int16_t>(it->value.GetUint());
  } else {
...
ParseOutputDeviceProfileFromJsonObject(const rapidjson::Value& value,
  ...
  if (driver_gain_db_it != value.MemberEnd()) {
    ...
    driver_gain_db = static_cast<float>(driver_gain_db_it->value.GetDouble());
  }
...
ParseInputDeviceProfileFromJsonObject(const rapidjson::Value& value, VolumeCurve volume_curve) {
  ...
  if (software_gain_db_it != value.MemberEnd()) {
    ...
    software_gain_db = static_cast<float>(software_gain_db_it->value.GetDouble());
  }
