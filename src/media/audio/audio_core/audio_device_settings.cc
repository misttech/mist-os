...
fuchsia::media::AudioGainInfo AudioDeviceSettings::GetGainInfo() const {
  ...
  // TODO(https://fxbug.dev/42110857): consider eliminating the acquisition of this lock.  In
  // theory, the only mutation of gain state happens during SetGainInfo, which is supposed to only
  // be called from the AudioDeviceManager, which should be functionally single threaded as it is
  // called only from the main service message loop. Since GetGainInfo should only be called from
  // the device manager as well, we should not need the settings_lock_ to observe the gain state
  // from this method.
  //
