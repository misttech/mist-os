...
bool DeviceReader::Create(fxl::WeakPtr<Controller> controller, uint32_t buffer_size_in_pages,
  ...
  auto status = zx::vmar::root_self()->allocate(ZX_VM_CAN_MAP_READ, 0u,
                                                static_cast<uint32_t>(buffer_size), &vmar, &addr);
  if (status != ZX_OK) {
  ...
  out_reader->reset(
      new DeviceReader(std::move(controller), static_cast<uint32_t>(buffer_size), std::move(vmar)));
  return true;
}
