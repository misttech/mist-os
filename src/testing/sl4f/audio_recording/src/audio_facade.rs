...
pub struct AudioFacade {
    vad_control: fidl_fuchsia_virtualaudio::ControlProxy,
    audio_output: VirtualOutput,
    audio_input: VirtualInput,
}
...
impl AudioFacade {
    ...
    pub async fn new() -> Result<AudioFacade, Error> {
        ...
        let mut audio_output =
            VirtualOutput::new(OUTPUT_SAMPLE_FORMAT, OUTPUT_CHANNELS, OUTPUT_FRAMES_PER_SECOND)?;
        audio_output.start_output(&vad_control).await?;

        let mut audio_input =
            VirtualInput::new(INPUT_SAMPLE_FORMAT, INPUT_CHANNELS, INPUT_FRAMES_PER_SECOND);
        audio_input.start_input(&vad_control).await?;

        Ok(AudioFacade { vad_control: vad_control, audio_output, audio_input })
    }
    ...
    pub async fn start_output_capture(&self) -> Result<(), Error> {
        self.audio_output.worker.lock().await.start_capturing()
    }
    ...
    pub async fn stop_output_capture(&self) -> Result<(), Error> {
        self.audio_output.worker.lock().await.stop_capturing()
    }
    ...
    pub async fn get_input_audio_size(&self, sample_index: usize) -> Result<usize, Error> {
        self.audio_input.get_track_size_in_bytes(sample_index).await
    }
    ...
    pub async fn get_output_audio_vec(&self) -> Result<Vec<u8>, Error> {
        self.audio_output.get_saved_output().await
    }

    /// Wait until input injection is completed.
    pub async fn wait_until_input_playing_is_finished(&self) -> Result<(), Error> {
        let fut = self.audio_input.worker.lock().await.get_injection_end_future();
    ...
    pub async fn put_input_audio(
        ...
        self.audio_input.append_input_track(sample_index, wave_data_vec).await;
    ...
    pub async fn clear_input_audio(&self, sample_index: usize) -> bool {
        self.audio_input.clear_input_track(sample_index).await
    }
    ...
    pub async fn start_input_injection(&self, sample_index: usize) -> Result<bool, Error> {
        self.audio_input.play(sample_index).await?;
        Ok(true)
    }
