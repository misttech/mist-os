                ...
                output_msg = output_events.try_next() => {
                    match output_msg? {
                        ...
                        Some(fidl_fuchsia_virtualaudio::DeviceEvent::OnStart { start_time }) => {
                            if self.capturing && last_timestamp > zx::MonotonicInstant::from_nanos(0) {
...
impl VirtualOutput {
    ...
    pub fn start_output(
        ...
        vad_control
            .add_device(&config, va_output_server, zx::MonotonicInstant::INFINITE)?
    ...
    #[allow(clippy::unused_io_amount)] // TODO(https://fxbug.dev/42176996)
...
impl VirtualInput {
    ...
    pub fn start_input(
        ...
        let (va_input_client, va_input_server) =
            create_endpoints::<fidl_fuchsia_virtualaudio::DeviceMarker>();
        vad_control
            .add_device(&config, va_input_server, zx::MonotonicInstant::INFINITE)?
            .map_err(|status| anyhow!("AddDevice returned error {:?}", status))?;
...
impl AudioFacade {
    async fn ensure_initialized(&self) -> Result<(), Error> {
        ...
        if !*(initialized) {
            ...
            self.vad_control.remove_all(zx::MonotonicInstant::INFINITE)?;
