...
"""Unit tests for honeydew.affordances.fuchsia_controller.bluetooth.profiles.bluetooth_gap.py"""
...
class BluetoothGapTest(unittest.TestCase):
    def setUp(self) -> None:
        super().setUp()
        ...
        self.bluetooth_gap_obj = bluetooth_gap.BluetoothGap(
    ...
    def test_sys_init(self, parameterized_dict: dict[str, Any]) -> None:
        ...
        # Check whether an `errors.BluetoothError` exception is raised when
        # calling `sys_init()` on a session that is already initialized.
        if parameterized_dict.get("session_initialized"):
            with self.assertRaises(errors.BluetoothStateError):
                self.bluetooth_gap_obj.sys_init()
        else:
            assert self.bluetooth_gap_obj._access_controller_proxy is not None
            assert (
                self.bluetooth_gap_obj._host_watcher_controller_proxy
                is not None
            )
            assert self.bluetooth_gap_obj._pairing_controller_proxy is not None
            assert not self.bluetooth_gap_obj.known_devices
    ...
    def test_connect_device(self, parameterized_dict: dict[str, Any]) -> None:
        ...
        self.bluetooth_gap_obj._access_controller_proxy.connect.assert_called_with(
            id=dummy_peer_id
        )
        self.assertEqual(
    ...
    def test_async_get_active_adapter_address(self) -> None:
        ...
        self.bluetooth_gap_obj._host_watcher_controller_proxy = mock.MagicMock()
        ...
        self.bluetooth_gap_obj._host_watcher_controller_proxy.watch = (
            mock.MagicMock(return_value=test)
        )
        res = asyncio.run(self.bluetooth_gap_obj._get_active_address())
    ...
    def test_get_known_remote_devices(self) -> None:
        ...
        self.bluetooth_gap_obj._access_controller_proxy = mock.MagicMock()
        data = self.bluetooth_gap_obj.get_known_remote_devices()
    ...
    def test_get_connected_devices(self) -> None:
        ...
        self.bluetooth_gap_obj._access_controller_proxy = mock.MagicMock()
        data = self.bluetooth_gap_obj.get_connected_devices()
    ...
    def test_pair_device(self, parameterized_dict: dict[str, Any]) -> None:
        ...
        self.bluetooth_gap_obj._access_controller_proxy.pair.assert_called_with(
    ...
    def test_request_discovery(
        ...
        if parameterized_dict.get("token") and parameterized_dict.get(
            ...
            assert self.bluetooth_gap_obj.discovery_token is None
