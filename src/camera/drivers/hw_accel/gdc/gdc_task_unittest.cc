...
template <typename T>
mock_mmio::Register& GetMockReg(mock_mmio::Region& registers) {
...
class TaskTest : public zxtest::Test {
 ...
 protected:
  ...
  // Returns a task id.
  uint32_t SetupForFrameProcessing(mock_mmio::Region& fake_regs) {
  ...
  void SetExpectations(mock_mmio::Region& fake_regs) {
...
TEST_F(TaskTest, InitTaskTest) {
  mock_mmio::Region fake_regs(sizeof(uint32_t), kNumberOfMmios);
