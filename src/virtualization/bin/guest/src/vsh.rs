...
async fn init_shell(socket: &mut fasync::Socket, args: Vec<String>) -> Result<Option<pty::RawPty>> {
    ...
    let maybe_raw_pty = pty::RawPty::new().await.map_err(|e| log::warn!("{e:?}")).ok().flatten();
        ...
        select! {
            ...
            result = event_handler => {
                ...
                if (events & !fpty::EVENT_WINDOW_SIZE) != 0 {
                    ...
                    // handling.
                    log::warn!("Unexpected Pty event received: {events:#x}");
                }
...
pub async fn handle_vsh<P: PlatformServices>(
    ...
    if !is_container && is_login_shell {
        ...
        .await
        {
            log::warn!("Failed to inject helper with error: {:?}", e);
        }
    }
    ...
    loop {
        select! {
            ...
            result = stdout_handler => {
                match result {
                    Ok(exit_code) => {
                        log::debug!("Guest process ended with exit code: {exit_code}");
                        return Ok(exit_code);
                    }
