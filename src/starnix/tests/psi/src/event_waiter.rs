...
/// file descriptor. Functions below build specializations backed by select,
/// poll and epoll. Thanks to unifying them under this generic interface, tests
/// can be written once and executed on the three of them, to prove that they
/// all behave coherently.
#[async_trait]
...
pub async fn make_select_waiter<'p>(
    puppet: &'p mut PuppetInstance,
    psi_fd: PuppetFileDescriptor,
) -> Box<dyn EventWaiter<'p> + 'p> {
    struct SelectEventWaiter<'p> {
        puppet: &'p mut PuppetInstance,
        psi_fd: PuppetFileDescriptor,
    }

    #[async_trait]
    impl<'p> EventWaiter<'p> for SelectEventWaiter<'p> {
        async fn wait(&mut self, timeout_ms: usize) -> bool {
            self.puppet.select_except(self.psi_fd, timeout_ms).await
        }

        async fn destroy(self: Box<Self>) {}
    }

    Box::new(SelectEventWaiter { puppet, psi_fd })
}
