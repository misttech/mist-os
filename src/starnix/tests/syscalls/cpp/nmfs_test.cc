...
class NmfsTest : public ::testing::Test {
 public:
  void SetUp() override {
    if (access("/sys/fs/nmfs", F_OK) == -1) {
      GTEST_SKIP() << "/sys/fs/nmfs not available, skipping...";
    }
    mount("nmfs", "/sys/fs/nmfs", "nmfs", 0, nullptr);
  }
};

TEST_F(NmfsTest, NmfsNetworkFileWriteFailure) {
  // Any string that is not the expected JSON format should not result in a write.
  EXPECT_FALSE(files::WriteFile("/sys/fs/nmfs/1", "test"))
      << "File contents must be in the proper JSON format";
  EXPECT_FALSE(files::WriteFile("/sys/fs/nmfs/1", "1"))
      << "File contents must be in the proper JSON format";

  // The name of the file must be an unsigned integer.
  EXPECT_FALSE(files::WriteFile("/sys/fs/nmfs/test",
                                R"({
    "version": "V1",
    "netid": 1,
    "mark": 56,
  ...
  EXPECT_FALSE(files::WriteFile("/sys/fs/nmfs/1.0",
                                R"({
    "version": "V1",
    "netid": 1,
    "mark": 56,
  ...
  EXPECT_FALSE(files::WriteFile("/sys/fs/nmfs/1",
                                R"({
    "version": "V9999",
    "netid": 1,
    "mark": 56,
...
TEST_F(NmfsTest, NmfsCannotCreateDir) { EXPECT_FALSE(files::CreateDirectory("/sys/fs/nmfs/2")); }

TEST_F(NmfsTest, NmfsNetworkFileWriteSuccessNoAddresses) {
  EXPECT_TRUE(files::WriteFile("/sys/fs/nmfs/3",
                               R"({
    "version": "V1",
    "netid": 3,
    "mark": 56,
  ...
  EXPECT_TRUE(files::ReadFileToString("/sys/fs/nmfs/3", &network_info))
      << "Network file should be readable";
  EXPECT_TRUE(files::WriteFile("/sys/fs/nmfs/3", network_info))
      << "Contents from read should be writeable to the same file";
}

TEST_F(NmfsTest, NmfsNetworkFileWriteSuccessV4Addresses) {
  EXPECT_TRUE(files::WriteFile("/sys/fs/nmfs/4",
                               R"({
    "version": "V1",
    "netid": 4,
    "mark": 56,
  ...
  // File output should be able to be rewritten to the file without formatting changes.
  std::string network_info;
  EXPECT_TRUE(files::ReadFileToString("/sys/fs/nmfs/4", &network_info))
      << "Network file should be readable";
  EXPECT_TRUE(files::WriteFile("/sys/fs/nmfs/4", network_info))
      << "Contents from read should be writeable to the same file";
}

TEST_F(NmfsTest, NmfsNetworkFileWriteSuccessV6Addresses) {
  EXPECT_TRUE(files::WriteFile("/sys/fs/nmfs/5",
                               R"({
    "version": "V1",
    "netid": 5,
    "mark": 56,
  ...
  // File output should be able to be rewritten to the file without formatting changes.
  std::string network_info;
  EXPECT_TRUE(files::ReadFileToString("/sys/fs/nmfs/5", &network_info))
      << "Network file should be readable";
