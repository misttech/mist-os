...
/// Reads /proc/cmdline within the starnix image
async fn read_proc_cmdline(realm: &RealmInstance) -> String {
    let read_flags = fuchsia_fs::PERM_READABLE;
    let file = fuchsia_fs::directory::open_file(
        realm.root.get_exposed_dir(),
        "/fs_root/proc/cmdline",
        read_flags,
    )
    .await
    .unwrap();
...
async fn test_no_errors_reboot_normal() {
    ...
    let cmdline = read_proc_cmdline(&realm_instance).await;
    assert!(
