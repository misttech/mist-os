...
impl<'a, T: AsRef<[u8]> + ?Sized> Parseable<NlaBuffer<&'a T>> for NsidAttribute {
    ...
    fn parse(buf: &NlaBuffer<&'a T>) -> Result<Self, NsidError> {
        ...
        Ok(match buf.kind() {
            NETNSA_NSID => Self::Id(
                parse_i32(payload)
                    .map_err(|error| NsidError::InvalidValue { kind: "NETNSA_NSID", error })?,
            ),
            NETNSA_PID => Self::Pid(
                parse_u32(payload)
                    .map_err(|error| NsidError::InvalidValue { kind: "NETNSA_PID", error })?,
            ),
            NETNSA_FD => Self::Fd(
                parse_u32(payload)
                    .map_err(|error| NsidError::InvalidValue { kind: "NETNSA_FD", error })?,
            ),
            NETNSA_TARGET_NSID => {
