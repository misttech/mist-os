    ...
    T: AsRef<[u8]> + ?Sized,
{
    ...
    fn parse_with_param(
        ...
        Ok(match kind {
            RouteLwEnCapType::Mpls => Self::Mpls(RouteMplsIpTunnel::parse(buf)?),
...
impl<'a, T> ParseableParametrized<NlaBuffer<&'a T>, RouteLwEnCapType> for VecRouteLwTunnelEncap
where
    T: AsRef<[u8]> + ?Sized,
{
    type Error = RouteError;
    fn parse_with_param(
        ...
        for nla in NlasIterator::new(buf.value()) {
            let nla = nla.map_err(|error| RouteError::InvalidRtaEncap { error, kind })?;
