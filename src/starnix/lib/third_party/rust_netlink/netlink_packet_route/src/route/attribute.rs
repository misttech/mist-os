    ...
    fn parse_with_param(
        ...
        Ok(match buf.kind() {
            RTA_DST => Self::Destination(
                RouteAddress::parse(address_family, payload)
                    .map_err(|error| RouteError::InvalidValue { kind: "RTA_DST", error })?,
            ),
            RTA_SRC => Self::Source(
                RouteAddress::parse(address_family, payload)
                    .map_err(|error| RouteError::InvalidValue { kind: "RTA_SRC", error })?,
            ),
            RTA_GATEWAY => Self::Gateway(
                RouteAddress::parse(address_family, payload)
                    .map_err(|error| RouteError::InvalidValue { kind: "RTA_GATEWAY", error })?,
            ),
            RTA_PREFSRC => Self::PrefSource(
                RouteAddress::parse(address_family, payload)
                    .map_err(|error| RouteError::InvalidValue { kind: "RTA_PREFSRC", error })?,
            ),
            RTA_VIA => Self::Via(
                RouteVia::parse(
            ...
            RTA_PREF => Self::Preference(
                parse_u8(payload)
                    .map_err(|error| RouteError::InvalidValue { kind: "RTA_PREF", error })?
                    .into(),
            ),
            ...
            RTA_EXPIRES => {
                if route_type == RouteType::Multicast {
                    Self::MulticastExpires(parse_u64(payload).map_err(|error| {
                        RouteError::InvalidValue { kind: "RTA_EXPIRES (multicast)", error }
                    })?)
                } else {
                    Self::Expires(
            ...
            RTA_UID => Self::Uid(
                parse_u32(payload)
                    .map_err(|error| RouteError::InvalidValue { kind: "RTA_UID", error })?,
            ),
            RTA_TTL_PROPAGATE => {
                Self::TtlPropagate(RouteMplsTtlPropagation::from(parse_u8(payload).map_err(
                    |error| RouteError::InvalidValue { kind: "RTA_TTL_PROPAGATE", error },
                )?))
            }
            RTA_ENCAP_TYPE => Self::EncapType(RouteLwEnCapType::from(
                parse_u16(payload)
                    .map_err(|error| RouteError::InvalidValue { kind: "RTA_ENCAP_TYPE", error })?,
            )),
            RTA_IIF => Self::Iif(
                parse_u32(payload)
                    .map_err(|error| RouteError::InvalidValue { kind: "RTA_IIF", error })?,
            ),
            RTA_OIF => Self::Oif(
                parse_u32(payload)
                    .map_err(|error| RouteError::InvalidValue { kind: "RTA_OIF", error })?,
            ),
            RTA_PRIORITY => Self::Priority(
                parse_u32(payload)
                    .map_err(|error| RouteError::InvalidValue { kind: "RTA_PRIORITY", error })?,
            ),
            RTA_FLOW => Self::Realm(RouteRealm::parse(payload)?),
            RTA_TABLE => Self::Table(
                parse_u32(payload)
                    .map_err(|error| RouteError::InvalidValue { kind: "RTA_TABLE", error })?,
            ),
            RTA_MARK => Self::Mark(
                parse_u32(payload)
                    .map_err(|error| RouteError::InvalidValue { kind: "RTA_MARK", error })?,
            ),
            ...
            _ => {
                Self::Other(DefaultNla::parse(buf).map_err(|error| RouteError::InvalidValue {
