...
impl MplsLabel {
    pub(crate) fn parse(payload: &[u8]) -> Result<Self, RouteError> {
        if payload.len() == 4 {
            Ok(Self::from(u32::from_be_bytes([payload[0], payload[1], payload[2], payload[3]])))
        } else {
            Err(RouteError::ParseMplsLabel { expected: 4, got: payload.len() })
        }
    }
}
...
impl From<u32> for MplsLabel {
    fn from(d: u32) -> Self {
        ...
        Self { label, traffic_class, bottom_of_stack, ttl }
    }
}
