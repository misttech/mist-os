...
use netlink_packet_utils::parsers::{parse_i32, parse_u16, parse_u32, parse_u8};
    ...
    fn parse(buf: &NlaBuffer<&'a T>) -> Result<Self, DecodeError> {
        ...
        Ok(match buf.kind() {
            ...
            IFLA_BOND_PORT_MII_STATUS => MiiStatus(
                parse_u8(payload).context("invalid IFLA_BOND_PORT_MII_STATUS value")?.into(),
            ),
            ...
            IFLA_BOND_PORT_STATE => BondPortState(
                parse_u8(payload).context("invalid IFLA_BOND_PORT_STATE value")?.into(),
            ),
