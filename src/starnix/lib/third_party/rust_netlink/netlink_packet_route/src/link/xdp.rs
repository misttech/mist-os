...
impl<'a, T: AsRef<[u8]> + ?Sized> Parseable<NlaBuffer<&'a T>> for LinkXdp {
    ...
    fn parse(nla: &NlaBuffer<&'a T>) -> Result<Self, DecodeError> {
        ...
        Ok(match nla.kind() as u32 {
            IFLA_XDP_FD => Self::Fd(parse_i32(payload).context("invalid IFLA_XDP_FD value")?),
            IFLA_XDP_ATTACHED => {
            ...
            IFLA_XDP_FLAGS => {
                Self::Flags(parse_u32(payload).context("invalid IFLA_XDP_FLAGS value")?)
            }
            IFLA_XDP_PROG_ID => {
                Self::ProgId(parse_u32(payload).context("invalid IFLA_XDP_PROG_ID value")?)
            }
            IFLA_XDP_DRV_PROG_ID => {
                Self::DrvProgId(parse_u32(payload).context("invalid IFLA_XDP_PROG_ID value")?)
            }
            IFLA_XDP_SKB_PROG_ID => {
                Self::SkbProgId(parse_u32(payload).context("invalid IFLA_XDP_PROG_ID value")?)
            }
            IFLA_XDP_HW_PROG_ID => {
                Self::HwProgId(parse_u32(payload).context("invalid IFLA_XDP_PROG_ID value")?)
            }
            IFLA_XDP_EXPECTED_FD => {
                Self::ExpectedFd(parse_u32(payload).context("invalid IFLA_XDP_PROG_ID value")?)
            }
            _ => Self::Other(
                DefaultNla::parse(nla).context(format!("unknown NLA type {}", nla.kind()))?,
            ),
        })
    }
}
