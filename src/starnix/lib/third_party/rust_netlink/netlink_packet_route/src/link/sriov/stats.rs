...
impl<'a, T: AsRef<[u8]> + ?Sized> Parseable<NlaBuffer<&'a T>> for VfStats {
    ...
    fn parse(buf: &NlaBuffer<&'a T>) -> Result<Self, DecodeError> {
        ...
        Ok(match buf.kind() {
            IFLA_VF_STATS_RX_PACKETS => Self::RxPackets(
                parse_u64(payload)
                    .context(format!("invalid IFLA_VF_STATS_RX_PACKETS value {payload:?}"))?,
            ),
            IFLA_VF_STATS_TX_PACKETS => Self::TxPackets(
                parse_u64(payload)
                    .context(format!("invalid IFLA_VF_STATS_TX_PACKETS value {payload:?}"))?,
            ),
            ...
            IFLA_VF_STATS_RX_DROPPED => Self::RxDropped(
                parse_u64(payload)
                    .context(format!("invalid IFLA_VF_STATS_RX_DROPPED value {payload:?}"))?,
            ),
            IFLA_VF_STATS_TX_DROPPED => Self::TxDropped(
                parse_u64(payload)
                    .context(format!("invalid IFLA_VF_STATS_TX_DROPPED value {payload:?}"))?,
            ),
            kind => Self::Other(
