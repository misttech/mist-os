...
impl Nla for InfoBridge {
    ...
    fn emit_value(&self, buffer: &mut [u8]) {
        match self {
            ...
            | Self::MultiBoolOpt(value) => NativeEndian::write_u64(buffer, *value),
            ...
            | Self::RootPathCost(value) => NativeEndian::write_u32(buffer, *value),

            Self::Priority(value)
            | Self::GroupFwdMask(value)
            | Self::RootPort(value)
            | Self::VlanDefaultPvid(value) => NativeEndian::write_u16(buffer, *value),

            Self::VlanProtocol(value) => BigEndian::write_u16(buffer, *value),

            Self::RootId(bridge_id) | Self::BridgeId(bridge_id) => bridge_id.emit(buffer),
    ...
    fn kind(&self) -> u16 {
        match self {
            ...
            Self::MulticastMembershipInterval(_) => IFLA_BR_MCAST_MEMBERSHIP_INTVL,
            Self::MulticastQuerierInterval(_) => IFLA_BR_MCAST_QUERIER_INTVL,
            Self::MulticastQueryInterval(_) => IFLA_BR_MCAST_QUERY_INTVL,
            Self::MulticastQueryResponseInterval(_) => IFLA_BR_MCAST_QUERY_RESPONSE_INTVL,
            ...
            Self::MulticastStartupQueryCount(_) => IFLA_BR_MCAST_STARTUP_QUERY_CNT,
            Self::MulticastLastMemberInterval(_) => IFLA_BR_MCAST_LAST_MEMBER_INTVL,
            Self::MulticastStartupQueryInterval(_) => IFLA_BR_MCAST_STARTUP_QUERY_INTVL,
            ...
            Self::MulticastQuerierState(_) => IFLA_BR_MCAST_QUERIER_STATE | NLA_F_NESTED,
...
impl<'a, T: AsRef<[u8]> + ?Sized> Parseable<NlaBuffer<&'a T>> for InfoBridge {
    ...
    fn parse(buf: &NlaBuffer<&'a T>) -> Result<Self, DecodeError> {
        ...
        Ok(match buf.kind() {
            ...
            IFLA_BR_HELLO_TIMER => {
                Self::HelloTimer(parse_u64(payload).context("invalid IFLA_BR_HELLO_TIMER value")?)
            }
            IFLA_BR_TCN_TIMER => {
                Self::TcnTimer(parse_u64(payload).context("invalid IFLA_BR_TCN_TIMER value")?)
            }
            ...
            IFLA_BR_GC_TIMER => {
                Self::GcTimer(parse_u64(payload).context("invalid IFLA_BR_GC_TIMER value")?)
            }
            IFLA_BR_MCAST_LAST_MEMBER_INTVL => Self::MulticastLastMemberInterval(
                parse_u64(payload).context("invalid IFLA_BR_MCAST_LAST_MEMBER_INTVL value")?,
            ),
            IFLA_BR_MCAST_MEMBERSHIP_INTVL => Self::MulticastMembershipInterval(
            ...
            IFLA_BR_MCAST_QUERY_RESPONSE_INTVL => Self::MulticastQueryResponseInterval(
            ...
            IFLA_BR_FORWARD_DELAY => Self::ForwardDelay(
                parse_u32(payload).context("invalid IFLA_BR_FORWARD_DELAY value")?,
            ),
            IFLA_BR_HELLO_TIME => {
                Self::HelloTime(parse_u32(payload).context("invalid IFLA_BR_HELLO_TIME value")?)
            }
            IFLA_BR_MAX_AGE => {
                Self::MaxAge(parse_u32(payload).context("invalid IFLA_BR_MAX_AGE value")?)
            }
            IFLA_BR_AGEING_TIME => {
                Self::AgeingTime(parse_u32(payload).context("invalid IFLA_BR_AGEING_TIME value")?)
            }
            IFLA_BR_STP_STATE => {
                Self::StpState(parse_u32(payload).context("invalid IFLA_BR_STP_STATE value")?)
            }
            ...
            IFLA_BR_MCAST_STARTUP_QUERY_CNT => Self::MulticastStartupQueryCount(
                parse_u32(payload).context("invalid IFLA_BR_MCAST_STARTUP_QUERY_CNT value")?,
            ),
            IFLA_BR_ROOT_PATH_COST => Self::RootPathCost(
                parse_u32(payload).context("invalid IFLA_BR_ROOT_PATH_COST value")?,
            ),
            IFLA_BR_PRIORITY => {
                Self::Priority(parse_u16(payload).context("invalid IFLA_BR_PRIORITY value")?)
            }
            IFLA_BR_VLAN_PROTOCOL => Self::VlanProtocol(
                parse_u16_be(payload).context("invalid IFLA_BR_VLAN_PROTOCOL value")?,
            ),
            ...
            IFLA_BR_GROUP_ADDR => {
                Self::GroupAddr(parse_mac(payload).context("invalid IFLA_BR_GROUP_ADDR value")?)
            }
            IFLA_BR_ROOT_PORT => {
                Self::RootPort(parse_u16(payload).context("invalid IFLA_BR_ROOT_PORT value")?)
            }
            ...
            IFLA_BR_VLAN_FILTERING => Self::VlanFiltering(
                parse_u8(payload).context("invalid IFLA_BR_VLAN_FILTERING value")? > 0,
            ),
            IFLA_BR_TOPOLOGY_CHANGE => Self::TopologyChange(
                parse_u8(payload).context("invalid IFLA_BR_TOPOLOGY_CHANGE value")?,
            ),
            ...
            IFLA_BR_MCAST_ROUTER => Self::MulticastRouter(
                parse_u8(payload).context("invalid IFLA_BR_MCAST_ROUTER value")?,
            ),
            ...
            IFLA_BR_MCAST_QUERIER => Self::MulticastQuerier(
                parse_u8(payload).context("invalid IFLA_BR_MCAST_QUERIER value")?,
            ),
            IFLA_BR_NF_CALL_IPTABLES => Self::NfCallIpTables(
                parse_u8(payload).context("invalid IFLA_BR_NF_CALL_IPTABLES value")?,
            ),
            ...
            IFLA_BR_MULTI_BOOLOPT => Self::MultiBoolOpt(
                parse_u64(payload).context("invalid IFLA_BR_MULTI_BOOLOPT value")?,
            ),
...
impl<T: AsRef<[u8]> + ?Sized> Parseable<BridgeIdBuffer<&T>> for BridgeId {
    ...
    fn parse(buf: &BridgeIdBuffer<&T>) -> Result<Self, DecodeError> {
        ...
        Ok(Self {
            ...
            address: parse_mac(buf.address()).context("invalid MAC address in BridgeId buffer")?,
        })
    }
}
