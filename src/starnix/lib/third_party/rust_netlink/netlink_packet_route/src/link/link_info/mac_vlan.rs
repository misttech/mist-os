...
impl<'a, T: AsRef<[u8]> + ?Sized> Parseable<NlaBuffer<&'a T>> for InfoMacVlan {
    ...
    fn parse(buf: &NlaBuffer<&'a T>) -> Result<Self, DecodeError> {
        ...
        Ok(match buf.kind() {
            IFLA_MACVLAN_MODE => {
                Mode(parse_u32(payload).context("invalid IFLA_MACVLAN_MODE value")?.into())
            }
            IFLA_MACVLAN_FLAGS => {
            ...
            kind => Other(
                DefaultNla::parse(buf)
                    .context(format!("unknown NLA type {kind} for IFLA_INFO_DATA(mac_vlan)"))?,
            ),
        })
    }
}
...
impl<'a, T: AsRef<[u8]> + ?Sized> Parseable<NlaBuffer<&'a T>> for InfoMacVtap {
    ...
    fn parse(buf: &NlaBuffer<&'a T>) -> Result<Self, DecodeError> {
        ...
        Ok(match buf.kind() {
            ...
            kind => Other(
                DefaultNla::parse(buf)
                    .context(format!("unknown NLA type {kind} for IFLA_INFO_DATA(mac_vtap)"))?,
            ),
        })
    }
}
