...
use netlink_packet_utils::parsers::{parse_u16_be, parse_u32, parse_u8};
...
impl Nla for InfoVxlan {
    fn value_len(&self) -> usize {
        match self {
            ...
            | Self::Limit(_)
            | Self::PortRange(_) => 4,
            Self::Local(bytes) | Self::Local6(bytes) | Self::Group(bytes) | Self::Group6(bytes) => {
                bytes.len()
            }
    ...
    fn emit_value(&self, buffer: &mut [u8]) {
        match self {
            ...
            Self::Gbp(_value) | Self::Gpe(_value) | Self::RemCsumNoPartial(_value) => (),
            Self::Tos(value) | Self::Ttl(value) | Self::Df(value) => buffer[0] = *value,
            ...
            Self::Local(value) | Self::Group(value) | Self::Group6(value) | Self::Local6(value) => {
...
impl<'a, T: AsRef<[u8]> + ?Sized> Parseable<NlaBuffer<&'a T>> for InfoVxlan {
    ...
    fn parse(buf: &NlaBuffer<&'a T>) -> Result<Self, DecodeError> {
        ...
        Ok(match buf.kind() {
            IFLA_VXLAN_ID => Self::Id(parse_u32(payload).context("invalid IFLA_VXLAN_ID value")?),
            IFLA_VXLAN_GROUP => Self::Group(payload.to_vec()),
            IFLA_VXLAN_GROUP6 => Self::Group6(payload.to_vec()),
            IFLA_VXLAN_LINK => {
                Self::Link(parse_u32(payload).context("invalid IFLA_VXLAN_LINK value")?)
            }
            IFLA_VXLAN_LOCAL => Self::Local(payload.to_vec()),
            IFLA_VXLAN_LOCAL6 => Self::Local6(payload.to_vec()),
            IFLA_VXLAN_TOS => Self::Tos(parse_u8(payload).context("invalid IFLA_VXLAN_TOS value")?),
            IFLA_VXLAN_TTL => Self::Ttl(parse_u8(payload).context("invalid IFLA_VXLAN_TTL value")?),
            IFLA_VXLAN_LABEL => {
                Self::Label(parse_u32(payload).context("invalid IFLA_VXLAN_LABEL value")?)
            }
            IFLA_VXLAN_LEARNING => {
                Self::Learning(parse_u8(payload).context("invalid IFLA_VXLAN_LEARNING value")? > 0)
            }
            IFLA_VXLAN_AGEING => {
                Self::Ageing(parse_u32(payload).context("invalid IFLA_VXLAN_AGEING value")?)
            }
            IFLA_VXLAN_LIMIT => {
                Self::Limit(parse_u32(payload).context("invalid IFLA_VXLAN_LIMIT value")?)
            }
            IFLA_VXLAN_PROXY => {
                Self::Proxy(parse_u8(payload).context("invalid IFLA_VXLAN_PROXY value")? > 0)
            }
            IFLA_VXLAN_RSC => {
                Self::Rsc(parse_u8(payload).context("invalid IFLA_VXLAN_RSC value")? > 0)
            }
            IFLA_VXLAN_L2MISS => {
                Self::L2Miss(parse_u8(payload).context("invalid IFLA_VXLAN_L2MISS value")? > 0)
            }
            IFLA_VXLAN_L3MISS => {
                Self::L3Miss(parse_u8(payload).context("invalid IFLA_VXLAN_L3MISS value")? > 0)
            }
            IFLA_VXLAN_COLLECT_METADATA => Self::CollectMetadata(
                parse_u8(payload).context("invalid IFLA_VXLAN_COLLECT_METADATA value")? > 0,
            ),
            ...
            IFLA_VXLAN_PORT => {
                Self::Port(parse_u16_be(payload).context("invalid IFLA_VXLAN_PORT value")?)
            }
            IFLA_VXLAN_UDP_CSUM => {
                Self::UDPCsum(parse_u8(payload).context("invalid IFLA_VXLAN_UDP_CSUM value")? > 0)
            }
            IFLA_VXLAN_UDP_ZERO_CSUM6_TX => Self::UDPZeroCsumTX(
                parse_u8(payload).context("invalid IFLA_VXLAN_UDP_ZERO_CSUM6_TX value")? > 0,
            ),
            IFLA_VXLAN_UDP_ZERO_CSUM6_RX => Self::UDPZeroCsumRX(
                parse_u8(payload).context("invalid IFLA_VXLAN_UDP_ZERO_CSUM6_RX value")? > 0,
            ),
            IFLA_VXLAN_REMCSUM_TX => Self::RemCsumTX(
                parse_u8(payload).context("invalid IFLA_VXLAN_REMCSUM_TX value")? > 0,
            ),
            IFLA_VXLAN_REMCSUM_RX => Self::RemCsumRX(
                parse_u8(payload).context("invalid IFLA_VXLAN_REMCSUM_RX value")? > 0,
            ),
            IFLA_VXLAN_DF => Self::Df(parse_u8(payload).context("invalid IFLA_VXLAN_DF value")?),
            ...
            IFLA_VXLAN_TTL_INHERIT => Self::TtlInherit(
                parse_u8(payload).context("invalid IFLA_VXLAN_TTL_INHERIT value")? > 0,
            ),
            IFLA_VXLAN_VNIFILTER => Self::Vnifilter(
                parse_u8(payload).context("invalid IFLA_VXLAN_VNIFILTER value")? > 0,
            ),
            IFLA_VXLAN_LOCALBYPASS => Self::Localbypass(
                parse_u8(payload).context("invalid IFLA_VXLAN_LOCALBYPASS value")? > 0,
            ),
