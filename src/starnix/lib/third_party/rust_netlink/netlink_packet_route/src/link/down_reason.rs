...
impl<'a, T: AsRef<[u8]> + ?Sized> Parseable<NlaBuffer<&'a T>> for LinkProtocolDownReason {
    type Error = DecodeError;
    fn parse(buf: &NlaBuffer<&'a T>) -> Result<Self, DecodeError> {
        let payload = buf.value();
        Ok(match buf.kind() {
            IFLA_PROTO_DOWN_REASON_MASK => Self::Mask(
                parse_u32(payload)
                    .context(format!("invalid IFLA_PROTO_DOWN_REASON_MASK {payload:?}"))?,
            ),
            IFLA_PROTO_DOWN_REASON_VALUE => Self::Value(
                parse_u32(payload)
                    .context(format!("invalid IFLA_PROTO_DOWN_REASON_MASK {payload:?}"))?,
            ),
...
impl Nla for LinkProtocolDownReason {
    ...
    fn emit_value(&self, buffer: &mut [u8]) {
        match self {
            Self::Value(v) | Self::Mask(v) => NativeEndian::write_u32(buffer, *v),
