...
impl Default for TcAction {
    fn default() -> Self {
        Self { tab: TCA_ACT_TAB, attributes: Vec::new() }
    }
}
...
impl<'a, T: AsRef<[u8]> + ?Sized> Parseable<NlaBuffer<&'a T>> for TcAction {
    ...
    fn parse(buf: &NlaBuffer<&'a T>) -> Result<Self, TcError> {
            ...
            attributes.push(match buf.kind() {
                TCA_ACT_KIND => {
                    kind = parse_string(payload)
                        .map_err(|error| TcError::ParseAction { kind: "TCA_ACT_KIND", error })?;
                ...
                TCA_ACT_INDEX => TcActionAttribute::Index(
                    parse_u32(payload)
                        .map_err(|error| TcError::ParseAction { kind: "TCA_ACT_INDEX", error })?,
                ),
                TCA_ACT_STATS => {
                    let mut nlas = vec![];
                    for nla in NlasIterator::new(payload) {
                ...
                TCA_ACT_IN_HW_COUNT => {
                    TcActionAttribute::InHwCount(parse_u32(payload).map_err(|error| {
                        TcError::ParseAction { kind: "TCA_ACT_IN_HW_COUNT", error }
                    })?)
                }
        ...
        Ok(Self { tab: buf.kind(), attributes })
    }
}
