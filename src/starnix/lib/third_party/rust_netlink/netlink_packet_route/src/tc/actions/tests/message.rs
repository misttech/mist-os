...
mod mirror {
    ...
    fn parse_message1_create() {
        let expected = TcActionMessage {
            header: TcActionMessageHeader { family: AddressFamily::Unspec },
                ...
                Actions(vec![
                    TcAction {
                        ...
                        attributes: vec![
                            Kind("mirred".into()),
                            Stats(vec![
                                Basic(TcStatsBasic { bytes: 0, packets: 0 }),
                                BasicHw(TcStatsBasic { bytes: 0, packets: 0 }),
                                Queue(TcStatsQueue {
                                    qlen: 0,
                                    backlog: 0,
                                    drops: 0,
...
fn tc_action_message_attribute_parse_back_example_action() {
    let orig = Actions(vec![TcAction { tab: 9999, attributes: vec![Kind("example".into())] }]);
    let mut buffer = vec![0; orig.buffer_len()];
    orig.emit(&mut buffer);
    let parsed =
        TcActionMessageAttribute::parse(&NlaBuffer::new_checked(buffer.as_slice()).unwrap())
...
fn tc_action_message_attribute_parse_back_multiple_example_action() {
    let orig = Actions(vec![
        TcAction { tab: 1111, attributes: vec![Kind("example1".into())] },
        TcAction {
            tab: 2222,
            attributes: vec![Kind("example2".into()), Index(42), Cookie(vec![1, 2, 3, 4, 5, 6])],
        },
    ]);
    let mut buffer = vec![0; orig.buffer_len()];
    orig.emit(&mut buffer);
...
fn tc_action_message_parse_back_example_value() {
    let orig = TcActionMessage {
        header: TcActionMessageHeader { family: AddressFamily::Alg },
        attributes: vec![
