...
impl InputEventsRelay {
    ...
    pub fn start_relays(
        ...
        kernel.kthreads.spawn_async_with_role(INPUT_RELAY_ROLE_NAME, async move |_: LockedAndTask<'_>| {
            // Because touch / mouse call watch() for polling new events, the calling sequence MUST
            // be:
            // 1. create future
            // 2. decrease counter
            // 3. await future
            ...
            //
            // Unlike touch/mouse, button events are coming from event stream and don't require the
            // counter to be decreased before awaiting the future. The counter is only decreased
            // after each button event is fully handled.
