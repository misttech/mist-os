...
impl HrTimerManager {
    pub fn new() -> HrTimerManagerHandle {
        Arc::new(Self {
            ...
            hr_timer_event: zx::Event::create(),
    ...
    pub fn init(self: &HrTimerManagerHandle, system_task: &CurrentTask) -> Result<(), Errno> {
        ...
        system_task.kernel().kthreads.spawn(move |_, system_task| {
            ...
            let _ = device_proxy
                .set_event(
                    HRTIMER_DEFAULT_ID,
                    self_ref.duplicate_timer_event().expect("duplicated hrtimer event"),
                    zx::Time::INFINITE,
                )
                .expect("set HrTimer driver event");
            let resolution_nsecs = get_hrtimer_resolution_nsecs(&device_proxy)
                .expect("hrtimer resolution nsecs should not be empty");

            self_ref.watch_new_hrtimer_loop(&system_task, &start_next_receiver, resolution_nsecs);
        });
    ...
    fn watch_new_hrtimer_loop(
            ...
            .is_ok()
        {
            ...
            let f = device_async_proxy.start_and_wait(
                HRTIMER_DEFAULT_ID,
    ...
    pub fn duplicate_timer_event(&self) -> Option<zx::Event> {
        self.hr_timer_event.duplicate_handle(zx::Rights::SAME_RIGHTS).ok()
    }
...
mod tests {
    ...
    async fn hr_timer_manager_add_timers() {
        ...
        assert!(hrtimer_manager.add_timer(None, &timer3, zx::MonotonicTime::from_nanos(3)).is_ok());
        assert!(hrtimer_manager.add_timer(None, &timer2, zx::MonotonicTime::from_nanos(2)).is_ok());
    ...
    async fn hr_timer_manager_remove_timers() {
        ...
        assert!(hrtimer_manager
            .add_timer(None, &timer1, zx::MonotonicTime::after(zx::Duration::from_seconds(1)))
            .is_ok());
    ...
    async fn hr_timer_manager_clear_heap() {
        ...
        assert!(hrtimer_manager
            .add_timer(None, &timer, zx::MonotonicTime::after(zx::Duration::from_seconds(1)))
            .is_ok());
