...
pub(super) fn file_system_init_security(
    ...
    Ok(FileSystemState { mount_options, label: OnceLock::new(), pending_entries: Mutex::default() })
}
...
pub(super) fn file_system_resolve_security(
    ...
    let pending_entries =
        std::mem::take(&mut *file_system.security_state.state.pending_entries.lock());
...
pub(super) struct FileSystemState {
    ...
    pub(super) pending_entries: Mutex<HashSet<WeakKey<DirEntry>>>,
}
