...
impl<T: TimeInterface> Timers<T> {
    ...
    fn wake_timers_impl(&self, from_receive_packet: bool) -> bool {
        ...
        // A note about the proper handing of the `from_receive_packet` flag.
        // This flag is set when `wake_timers_impl` is being called from an
        // Executor after having received a port packet generated by an
        // async-wait operation on the timer.
        //
        // We only ever want to have either zero or one async wait operation in
        // flight.  When `inner.async_wait` flag is true, it indicates that we
        // have an async wait operation "in-flight".  When `wake_timers_impl` is
        // called in response to receiving the expected port packet, we need to
        // clear the flag, but exactly _where_ we clear it is important.
        //
        // While processing the queue of timers, we are going to lock, grab the
        // head of the queue if it has expired, then drop the lock and perform
        // the callback.  We will repeat this operation until there are no
        // timers left in in the queue, or when all timers left in the queue are
        // in the future.
        //
        // We need to clear it when we are in the inner.lock, but _only after we
        // are completely finished_.  If we clear the flag in a situation where
        // we are going to drop the lock and return a waker object, the
        // following can happen.
        //
        // 1) We clear the flag.
        // 2) We drop the lock.
        // 3) While outside of the lock, something else adds a new timer to the
        //    queue and calls set_timer.
        // 4) set_timer sees that there is no async wait operation in flight so
        //    it adds one.
        // 5) Switching back to the loop, we re-acquire the lock and clear the
        //    flag again.
        // 6) Eventually, we get to the point where there is a timer in the
        //    future at the front of the queue.  We call set_timer, but have
        //    just cleared the flag meaning that we think we need to queue
        //    another async wait operation, so we do.
        // 7) Now there are two async waits in flight.  Each one of them is
        //    going to service the queue, reset the underlying zircon timer, and
        //    (because they clear the flag) queue another async wait.
        //
        // As long as there are timers in the queue, and new ones being added at
        // step #3, we can continue to build up in-flight async wait operations.
        // The number of them cannot drop back down to zero until we get to the
        // point that there are no timers in the queue anymore.
        //
        // So: When clearing the flag, make sure we only do it in situations
        // where the packet callback is actually finished, just before we
        // optionally set a new timer, then drop the lock and break out of the
        // loop.
        //
        loop {
            let waker = {
                ...
                if deadline.is_some_and(|d| d <= now) {
                    ...
                    // We are now finished (one way or the other).  If this is a
                    // callback caused by processing an async wait packet, we
                    // can go ahead and clear the flag now. After that, if we
                    // still have a timer (and therefore a deadline) in the
                    // queue in the future, go ahead and setup our zircon timer
                    // again.
                    if from_receive_packet {
                        inner.async_wait = false;
                    }
                    if let Some(deadline) = deadline {
                        self.set_timer(&mut inner, deadline);
                    }
                    break;
                }
            };
