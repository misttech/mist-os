...
impl DefineSubsystemConfiguration<(&Vec<ProductTeeClient>, &Vec<uuid::Uuid>)> for TeeClientsConfig {
    fn define_configuration(
        ...
        // Configure the tee_manager based on whether the board provided GUIDs to serve from it
        if !tee_trusted_app_guids.is_empty() {
            ...
            // tee_manager needs:
            // - its AIB (which currently only contains a shard)
            // - its config_data (which is generated by this assembly module)
            // - the tee_manager package (which is currently defined by the board)
            // TODO(b/358614540): make this assembly subsystem also control whether the package
            // is included
            builder.platform_bundle("tee_manager_shard_only");
        ...
        // Now hook up the clients of tee_manager
...
mod tests {
    ...
    fn test_define_configuration() {
        ...
        // Verify that we hooked up tee_manager correctly, both:
        // - config data
        // - core shard
        ...
        assert!(completed_configuration.bundles.contains("tee_manager_shard_only"));
    }
}
