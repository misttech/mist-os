...
// We implement a custom walk so that we don't append 'map' to the destination.
impl WalkPaths for InputBundlesConfig {
    fn walk_paths_with_dest<F: WalkPathsFn>(
        &mut self,
        found: &mut F,
        dest: Utf8PathBuf,
    ) -> Result<()> {
        self.map.walk_paths_with_dest(found, dest)
    }
}

/// The list of BIBs is either a map which is keyed by the BIB name (most of
/// the time), or a vector of BIBs (only when generated by the build system).
#[derive(Debug, Deserialize)]
#[serde(untagged)]
enum MapOrVecInputBundles {
    /// A map of BIB directories, keyed by their name.
    Map(BTreeMap<String, DirectoryPathBuf>),

    /// A list of BIB directories.
    Vec(Vec<DirectoryPathBuf>),
}

impl Default for MapOrVecInputBundles {
    fn default() -> Self {
        Self::Map(BTreeMap::default())
    }
}

/// Convert the list of BIBs to a map with names.
fn convert_to_map(map_or_vec: MapOrVecInputBundles) -> BTreeMap<String, DirectoryPathBuf> {
    match map_or_vec {
        MapOrVecInputBundles::Map(map) => map,
        // The key in the map defaults to the index in the vector.
        MapOrVecInputBundles::Vec(vec) => {
            vec.into_iter().enumerate().map(|(i, s)| (i.to_string(), s)).collect()
        }
    }
}

impl From<MapOrVecInputBundles> for InputBundlesConfig {
    fn from(map_or_vec: MapOrVecInputBundles) -> Self {
        InputBundlesConfig { map: convert_to_map(map_or_vec) }
    }
}

impl BoardInformation {
    /// Add the names of the BIBs to the mmap.
    pub fn add_bib_names(mut self) -> Result<Self> {
        let map: BTreeMap<String, DirectoryPathBuf> = self
            .input_bundles
            .map
            .into_iter()
            .map(|(key, dir)| {
                let bib = BoardInputBundle::from_dir(&dir)?;
                let name = bib.name.unwrap_or(key);
                Ok::<(String, DirectoryPathBuf), anyhow::Error>((name, dir))
            })
            .collect::<Result<BTreeMap<String, DirectoryPathBuf>>>()?;
        self.input_bundles.map = map;
        Ok(self)
    }
}
