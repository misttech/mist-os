...
mod tests {
    ...
    fn decode_ints() {
        ...
        assert_decoded::<u16_le>(&mut chunks!(0x34 0x12 0x00 0x00 0x00 0x00 0x00 0x00), |x| {
            assert_eq!(*x, 0x1234u16)
        });
        assert_decoded::<i16_le>(&mut chunks!(0xcc 0xed 0x00 0x00 0x00 0x00 0x00 0x00), |x| {
            assert_eq!(*x, -0x1234i16)
        });

        assert_decoded::<u32_le>(&mut chunks!(0x78 0x56 0x34 0x12 0x00 0x00 0x00 0x00), |x| {
            assert_eq!(*x, 0x12345678u32)
        });
        assert_decoded::<i32_le>(&mut chunks!(0x88 0xa9 0xcb 0xed 0x00 0x00 0x00 0x00), |x| {
            assert_eq!(*x, -0x12345678i32)
        });

        assert_decoded::<u64_le>(&mut chunks!(0xf0 0xde 0xbc 0x9a 0x78 0x56 0x34 0x12), |x| {
            assert_eq!(*x, 0x123456789abcdef0u64)
        });
        assert_decoded::<i64_le>(&mut chunks!(0x10 0x21 0x43 0x65 0x87 0xa9 0xcb 0xed), |x| {
            assert_eq!(*x, -0x123456789abcdef0i64)
        });
    }

    #[test]
    fn decode_floats() {
        assert_decoded::<f32_le>(&mut chunks!(0xdb 0x0f 0x49 0x40 0x00 0x00 0x00 0x00), |x| {
            assert_eq!(*x, ::core::f32::consts::PI)
        });
        assert_decoded::<f64_le>(&mut chunks!(0x18 0x2d 0x44 0x54 0xfb 0x21 0x09 0x40), |x| {
