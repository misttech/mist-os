...
use fuchsia_zircon::{AsHandleRef, DurationNum, MessageBuf, MonotonicTime, Signals};
...
fn two_way_strict_sync_send() {
    run_two_way_sync(
        |client| client.strict_two_way(MonotonicTime::after(2.seconds())),
        &[
            0x02, 0x00, 0x00, 0x01, //
...
fn two_way_strict_err_sync_send() {
    run_two_way_sync(
        |client| client.strict_two_way_err(MonotonicTime::after(2.seconds())),
        &[
            0x02, 0x00, 0x00, 0x01, //
...
fn two_way_flexible_sync_send() {
    run_two_way_sync(
        |client| client.flexible_two_way(MonotonicTime::after(2.seconds())),
        &[
            0x02, 0x00, 0x80, 0x01, //
...
fn two_way_flexible_err_sync_send() {
    run_two_way_sync(
        |client| client.flexible_two_way_err(MonotonicTime::after(2.seconds())),
        &[
            0x02, 0x00, 0x80, 0x01, //
...
fn recieve_unknown_event_strict_sync() {
    ...
    let err = client
        .wait_for_event(MonotonicTime::after(2.seconds()))
