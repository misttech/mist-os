...
bool FlatlandView::Init(fidl::UnownedClientEnd<fuchsia_io::Directory> service_directory,
  ...
  if (set_debug_name_result.is_error()) {
    FX_LOGS(ERROR) << "Failed to set debug name: "
                   << set_debug_name_result.error_value().FormatDescription();
  }

  fit::result<fidl::OneWayError> create_transform_result =
      flatland_->CreateTransform(kRootTransform);
  if (create_transform_result.is_error()) {
    FX_LOGS(ERROR) << "Failed to call CreateTransform: "
                   << create_transform_result.error_value().FormatDescription();
    return false;
  }

  fit::result<fidl::OneWayError> set_root_transform_result =
      flatland_->SetRootTransform(kRootTransform);
  if (set_root_transform_result.is_error()) {
    FX_LOGS(ERROR) << "Failed to call SetRootTransform: "
                   << set_root_transform_result.error_value().FormatDescription();
    return false;
  }
  ...
  if (create_view2_result.is_error()) {
    FX_LOGS(ERROR) << "Failed to call CreateView2: "
                   << set_root_transform_result.error_value().FormatDescription();
    return false;
  }
...
void FlatlandView::OnGetLayout(fuchsia_ui_composition::LayoutInfo info) {
  ...
  if (viewport_creation_token_.value().is_valid()) {
    // The first time that we receive layout information, create a viewport using the token that was
    // stashed during Init(). External code will attach a view to this viewport via the token
    // obtained from TakeChildViewCreationToken().
    ...
    if (create_viewport_result.is_error()) {
      FX_LOGS(ERROR) << "Failed to call CreateViewport(): "
                     << create_viewport_result.error_value().FormatDescription();
      return;
    }
    ...
    if (set_content_result.is_error()) {
      FX_LOGS(ERROR) << "Failed to call SetContent(): "
                     << set_content_result.error_value().FormatDescription();
      return;
    }
  } else {
...
void FlatlandView::Present() {
  ...
  if (present_result.is_error()) {
    FX_LOGS(ERROR) << "Failed to call Present(): "
                   << present_result.error_value().FormatDescription();
  }
}

void FlatlandView::OnFramePresented(
    fidl::Event<fuchsia_ui_composition::Flatland::OnFramePresented>& event) {}

void FlatlandView::OnNextFrameBegin(
    fidl::Event<fuchsia_ui_composition::Flatland::OnNextFrameBegin>& event) {
...
void FlatlandViewProviderService::CreateView2(CreateView2Request& request,
