...
impl Storage {
    pub fn new() -> Self {
        Self { persistent_objects: PersistentObjects::new() }
    }
    ...
    pub fn get_object_info(&self, object: ObjectHandle) -> ObjectInfo {
        ...
        self.persistent_objects.get(object).lock().get_info()
    }
    ...
    pub fn restrict_object_usage(&self, object: ObjectHandle, usage: Usage) {
        ...
        self.persistent_objects.get(object).lock().object.lock().restrict_usage(usage)
    }
}
...
impl Storage {
    ...
    pub fn get_object_buffer_attribute<'a>(
        ...
        if is_transient_handle(object) {
            ...
            copy_from_key(&self.persistent_objects.get(object).lock().object.lock().key, buffer)
        }
    }
    ...
    pub fn get_object_value_attribute(
        ...
        if is_transient_handle(object) {
            ...
            copy_from_key(&self.persistent_objects.get(object).lock().object.lock().key)
        }
    }
    ...
    pub fn free_transient_object(&self, object: ObjectHandle) {
        assert!(is_transient_handle(object));
        unimplemented!()
    }

    pub fn reset_transient_object(&self, object: ObjectHandle) {
        assert!(is_transient_handle(object));
        unimplemented!()
    }
...
impl Storage {
    ...
    pub fn create_persistent_object(
        ...
        let (key, usage, base_flags) = if attribute_src.is_null() {
            ...
            let view_guard = self.persistent_objects.get(attribute_src);
    ...
    pub fn read_object_data<'a>(
        ...
        assert!(is_persistent_handle(object));
        self.persistent_objects.get(object).lock().read_data(buffer)
    }
    ...
    pub fn write_object_data(&self, object: ObjectHandle, buffer: &[u8]) -> TeeResult {
        assert!(is_persistent_handle(object));
        self.persistent_objects.get(object).lock().write_data(buffer)
    }
    ...
    pub fn truncate_object_data(&self, object: ObjectHandle, size: usize) -> TeeResult {
        assert!(is_persistent_handle(object));
        self.persistent_objects.get(object).lock().truncate_data(size)
    }
    ...
    pub fn seek_data_object(
        ...
        assert!(is_persistent_handle(object));
        self.persistent_objects.get(object).lock().seek_data(offset, whence)
    }
}
