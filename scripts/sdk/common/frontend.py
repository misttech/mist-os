...
_FUCHSIA_ROOT = os.path.dirname(  # $root
    os.path.dirname(os.path.dirname(_SCRIPT_DIR))  # scripts  # sdk
)  # common
...
class Frontend(object):
    ...
    def dest(self, *args):
        ...
        """
        if (
            os.path.commonprefix([os.path.realpath(args[0]), self.output])
            == self.output
        ):
    ...
    def copy_file(
        self, filename, root="", destination="", result=[], allow_overwrite=True
    ):
    ...
    def copy_files(
        self, files, root="", destination="", result=[], allow_overwrite=True
    ):
        ...
        with self._create_archive_dir() as archive_dir:
            ...
            for part in manifest["parts"]:
                ...
                if type == "version_history":
                    getattr(self, "install_%s_atom" % type, self._handle_atom)(
                        atom, part
                    )
                    continue
