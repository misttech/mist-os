...
#include <lib/mistos/elfldltl/segment-with-vmo.h>
...
TEST(ElfldltlSegmentWithVmoTests, MutableMemoryStore) {
  ...
  for (size_type offset = 0; offset < kFileSize; offset += sizeof(size_type)) {
    ...
    EXPECT_EQ(value, unrelocated_value(offset), " at offset %lu", offset);
  }
  ...
  for (size_type offset = 0; offset < kMutable1Size; offset += sizeof(size_type)) {
    ...
    switch (addr) {
      default:
        EXPECT_EQ(value, unrelocated_value(addr), " at offset %lu for address %lu", offset, addr);
        break;
