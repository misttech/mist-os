...
#ifndef ZIRCON_KERNEL_LIB_MISTOS_ELFLDLTL_INCLUDE_LIB_MISTOS_ELFLDLTL_VMAR_LOADER_H_
#define ZIRCON_KERNEL_LIB_MISTOS_ELFLDLTL_INCLUDE_LIB_MISTOS_ELFLDLTL_VMAR_LOADER_H_
...
class StaticOrDynExecutableVmarLoader;
...
/// elfldltl::StaticOrDynExecutableVmarLoader is like elfldltl::RemoteVmarLoader but
/// works for loading static linked executables, mainly for MistOS/Linux unmodified programs.
class StaticOrDynExecutableVmarLoader : public VmarLoader {
 public:
  explicit StaticOrDynExecutableVmarLoader(const zx::vmar& vmar, bool dyn)
      : VmarLoader(vmar), dyn_(dyn) {}

  template <class Diagnostics, class LoadInfo>
  [[nodiscard]] bool Load(Diagnostics& diag, const LoadInfo& load_info, zx::unowned_vmo vmo) {
    if (!dyn_ && !load_image_vmar_) {
      zx_info_vmar_t info;
      zx_status_t status = vmar_->get_info(ZX_INFO_VMAR, &info, sizeof(info), nullptr, nullptr);
      if (status != ZX_OK) {
        diag.SystemError("cannot get_info from vmar: ", ZirconError{status});
        return false;
      }

      size_t load_offset = load_info.vaddr_start() - info.base;
      // Allocate wasn't called yet, so do it now.  If it returns true after
      // failure because Diagnostics::SystemError returned true, then still
      // bail out early but also return true.
      if (bool ok = Allocate(diag, load_info, load_offset); !ok || !load_image_vmar_) {
        return ok;
      }
    }
    return VmarLoader::Load<PartialPagePolicy::kZeroInVmo>(diag, load_info, vmo->borrow());
...
#endif  // ZIRCON_KERNEL_LIB_MISTOS_ELFLDLTL_INCLUDE_LIB_MISTOS_ELFLDLTL_VMAR_LOADER_H_
