...
void ExpectVmoIsCloned(const zx::vmo& vmo, const zx::vmo& parent) {
  ...
  EXPECT_TRUE(info.flags & ZX_INFO_VMO_IS_COW_CLONE, "flags: 0x%x", info.flags);
}
...
void ExpectVmoIsNotCloned(const zx::vmo& vmo) {
  ...
  EXPECT_FALSE(info.flags & ZX_INFO_VMO_IS_COW_CLONE, "flags: 0x%x", info.flags);
}
...
void TestCloning() {
  ...
  // kSecondItemOnPageBoundary.
  {
    ...
    ASSERT_NO_FATAL_FAILURE(
        OpenTestDataZbi(TestDataZbiType::kSecondItemOnPageBoundary, &buff, &size));
    ...
    // item offset % ZX_PAGE_SIZE == 0, and so we do not expect a clone.
    {
      ...
      auto result = created_view.take_error();
      ASSERT_FALSE(result.is_error(), "%s", ViewErrorString(result.error_value()).c_str());
    }
  ...
  // kMultipleSmallItems
  {
    ...
    // a clone with a single discard item.
    {
      ...
      // byte-for-byte equality.
      {
        ...
        ASSERT_FALSE(result.is_error(), "%s", zbitl::ViewErrorString(result.error_value()).c_str());
...
void TestLargeFileDecompression() {
  ...
  fbl::String compressed{kLargeZst, sizeof(kLargeZst) - 1};
...

  {
    ...
    ASSERT_FALSE(result.is_error(), "%s", zbitl::ViewCopyErrorString(result.error_value()).c_str());
...
void TestInheritedResizability() {
  ...
  // Resizable if parent was resizable.
  {
    ...
    typename TestTraits::Context context;
    ASSERT_NO_FATAL_FAILURE(
        TestTraits::template Create</*Resizable=*/true>(std::move(buff), size, &context));
    zbitl::View view(context.TakeStorage());
  ...
  // Non-resizable if parent was non-resizable.
  {
    ...
    typename TestTraits::Context context;
    ASSERT_NO_FATAL_FAILURE(
        TestTraits::template Create</*Resizable=*/false>(std::move(buff), size, &context));
    zbitl::View view(context.TakeStorage());
