...
TEST(ZbitlViewFblByteArrayTests, BoundsChecking) {
  ...
  ktl::span<const char> data;
  size_t size = 0;
  ASSERT_NO_FATAL_FAILURE(OpenTestDataZbi(TestDataZbiType::kOneItem, &data, &size));

  typename TestTraits::Context context;
  ASSERT_NO_FATAL_FAILURE(TestTraits::Create(std::move(data), size, &context));
