...
#ifndef ZIRCON_KERNEL_LIB_MISTOS_ZBITL_TESTS_TESTS_H_
#define ZIRCON_KERNEL_LIB_MISTOS_ZBITL_TESTS_TESTS_H_

#include <lib/mistos/zbitl/error-string.h>
...
// While it is convenient to use fbl::String as a container in representing ZBI
// content, we alias the type to convey that it need not necessarily represent
// text.
using Bytes = fbl::String;
...
inline void TestIteration(TestDataZbiType type) {
  ...
  size_t size = 0;
  ASSERT_NO_FATAL_FAILURE(OpenTestDataZbi(type, &buff, &size));

  typename TestTraits::Context context;
  ASSERT_NO_FATAL_FAILURE(TestTraits::Create(std::move(buff), size, &context));
  zbitl::View view(context.TakeStorage());

  auto container_result = view.container_header();
  ASSERT_FALSE(container_result.is_error(), "%s",
               ViewErrorString(container_result.error_value()).c_str());
  ...
  for (auto it = view.begin(); it != view.end(); ++it) {
    ...
    EXPECT_TRUE(flags & ZBI_FLAGS_VERSION, "flags: 0x%x", flags);

    // Let's verify CRC32 while we're at it.
    auto crc_result = view.CheckCrc32(it);
    ASSERT_FALSE(crc_result.is_error(), "%s", ViewErrorString(crc_result.error_value()).c_str());
...
void TestMutation(TestDataZbiType type) {
  ktl::span<const char> buff;
  size_t size = 0;
  OpenTestDataZbi(type, &buff, &size);
...
void TestCopyCreation(TestDataZbiType type, ItemCopyMode mode) {
  ...
  for (auto it = view.begin(); it != view.end(); ++it, ++idx) {
    ...
    ASSERT_TRUE(result.is_ok(), "item %zu: %s", idx,
                ViewCopyErrorString(result.error_value()).c_str());
...
void TestCopyCreationByByteRange(TestDataZbiType type) {
  ...
  for (auto it = view.begin(); it != view.end(); ++it, ++idx) {
    ...
    EXPECT_FALSE(result.is_error(), "%s", ViewCopyErrorString(result.error_value()).c_str());
    ...
    Bytes tmp;
    ASSERT_NO_FATAL_FAILURE(GetExpectedPayload(type, idx, &tmp));
    Bytes expected(to_offset, '\0');
    expected += tmp;
...
void TestCopyCreationByIteratorRange(TestDataZbiType type) {
  ...
  typename TestTraits::Context context;
  ASSERT_NO_FATAL_FAILURE(TestTraits::Create(std::move(buff), buff.size(), &context));
  zbitl::View view(context.TakeStorage());

  // [begin(), begin())
  {
    auto copy_result = view.Copy(view.begin(), view.begin());
    EXPECT_FALSE(copy_result.is_error(), "%s",
                 ViewCopyErrorString(copy_result.error_value()).c_str());
...
void TestCopyingIntoSmallStorage() {
  ktl::span<const char> buff;
  size_t size = 0;
  ASSERT_NO_FATAL_FAILURE(OpenTestDataZbi(TestDataZbiType::kOneItem, &buff, &size));

  typename SrcTestTraits::Context src_context;
  ASSERT_NO_FATAL_FAILURE(SrcTestTraits::Create(std::move(buff), buff.size(), &src_context));
  ...
  ASSERT_FALSE(copy_result.is_error(), "%s",
               ViewCopyErrorString(std::move(copy_result).error_value()).c_str());
  ...
  auto result = view.take_error();
  ASSERT_FALSE(result.is_error(), "%s", ViewErrorString(std::move(result).error_value()).c_str());
}
...
void TestCopying(TestDataZbiType type, ItemCopyMode mode) {
  ...
  ASSERT_NO_FATAL_FAILURE(SrcTestTraits::Create(std::move(buff), size, &context));
  zbitl::View view(context.TakeStorage());
...
void TestAppending() {
  ...
  // sufficient size to hold a trivial ZBI container header).
  {
    ...
    ASSERT_FALSE(clear_result.is_error(), "%s",
                 ViewErrorString(std::move(clear_result).error_value()).c_str());
  ...
  for (const Bytes &bytes : to_append) {
    ...
    ASSERT_FALSE(append_result.is_error(), "bytes = \"%s\": %s", bytes.c_str(),
                 ViewErrorString(std::move(append_result).error_value()).c_str());
  }
    ...
    for (size_t i = 0; i < std::size(to_append) && it != image.end(); ++i, ++it) {
      ...
      switch (variation) {
        case 0: {  // append-with-payload
          ...
          ASSERT_FALSE(crc_result.is_error(), "%s",
                       zbitl::ViewErrorString(crc_result.error_value()).c_str());
      ...
      EXPECT_EQ(expected, fbl::String(actual.data(), expected.size()));
  ...
  if constexpr (!TestTraits::kExpectExtensibility) {
    ...
    {
      auto result = image.Append(zbi_header_t{.type = kItemType}, zbitl::ByteView{});
      EXPECT_FALSE(result.is_error(), "%s",
                   ViewErrorString(std::move(result).error_value()).c_str());
    }
    {
      auto result = image.Append(zbi_header_t{.type = kItemType, .length = 0});
      EXPECT_FALSE(result.is_error(), "%s",
...

    {
      ...
      ASSERT_FALSE(result.is_error(), "%s",
                   ViewErrorString(std::move(result).error_value()).c_str());
      ...
      ASSERT_FALSE(trim_result.is_error(), "%s",
                   ViewErrorString(std::move(trim_result).error_value()).c_str());
...

  {
    ...
    EXPECT_FALSE(result.is_error(), "%s", ViewErrorString(std::move(result).error_value()).c_str());
  }
...
void TestExtending() {
  ktl::span<const char> buff;
  size_t size = 0;
  ASSERT_NO_FATAL_FAILURE(OpenTestDataZbi(TestDataZbiType::kMultipleSmallItems, &buff, &size));

  typename SrcTestTraits::Context src_context;
  ASSERT_NO_FATAL_FAILURE(SrcTestTraits::Create(std::move(buff), size, &src_context));
  zbitl::View view(src_context.TakeStorage());
  ...
  // [begin(), begin() + 1)
  {
    ...
    EXPECT_FALSE(extend_result.is_error(), "%s",
                 ViewCopyErrorString(std::move(extend_result).error_value()).c_str());
...
#endif  // ZIRCON_KERNEL_LIB_MISTOS_ZBITL_TESTS_TESTS_H_
