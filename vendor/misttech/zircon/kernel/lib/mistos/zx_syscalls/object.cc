...
class SubsetVmoInfoWriter : public VmoInfoWriter {
 public:
  SubsetVmoInfoWriter(T* out) : out_(out) {}
  ...
  zx_status_t Write(const zx_info_vmo_t& vmo, size_t offset) override {
    ...
    memcpy(out_ + (offset + base_offset_), versioned_vmo, sizeof(T));
...
zx_status_t zx_object_get_info(zx_handle_t handle, uint32_t topic, void* _buffer,
  ...
  switch (topic) {
    case ZX_INFO_HANDLE_VALID: {
      ...
      return handle_table(up).GetDispatcherWithRightsNoPolicyCheck(handle, 0, &generic_dispatcher,
                                                                   nullptr);
    }
    case ZX_INFO_HANDLE_BASIC: {
      ...
      zx_rights_t rights;
      auto status = handle_table(up).GetDispatcherAndRights(*up, handle, &dispatcher, &rights);
      if (status != ZX_OK)
        return status;
    ...
    case ZX_INFO_PROCESS: {
      ...
      auto error =
          handle_table(up).GetDispatcherWithRights(*up, handle, ZX_RIGHT_INSPECT, &process);
      if (error != ZX_OK)
        return error;
    ...
    case ZX_INFO_VMAR: {
      ...
      zx_status_t status =
          handle_table(up).GetDispatcherWithRights(*up, handle, ZX_RIGHT_INSPECT, &vmar);
      if (status != ZX_OK)
        return status;
    ...
    case ZX_INFO_HANDLE_COUNT: {
      fbl::RefPtr<Dispatcher> dispatcher;
      auto status =
          handle_table(up).GetDispatcherWithRights(*up, handle, ZX_RIGHT_INSPECT, &dispatcher);
      if (status != ZX_OK)
        return status;
...
zx_status_t zx_object_get_property(zx_handle_t handle_value, uint32_t property, void* _value,
                                   size_t size) {
  if (!_value)
    return ZX_ERR_INVALID_ARGS;
  ...
  fbl::RefPtr<Dispatcher> dispatcher;
  zx_status_t status = handle_table(up).GetDispatcherWithRights(*up, handle_value,
                                                                ZX_RIGHT_GET_PROPERTY, &dispatcher);
  if (status != ZX_OK)
    return status;
  switch (property) {
    case ZX_PROP_NAME: {
    ...
    case ZX_PROP_VMO_CONTENT_SIZE: {
      ...
      *(reinterpret_cast<uint64_t*>(_value)) = value;
...
zx_status_t zx_object_set_property(zx_handle_t handle_value, uint32_t property, const void* _value,
                                   size_t size) {
  if (!_value)
    return ZX_ERR_INVALID_ARGS;
  ...
  const zx_status_t get_dispatcher_status = handle_table(up).GetDispatcherWithRights(
  ...
  switch (property) {
    ...
    case ZX_PROP_VMO_CONTENT_SIZE: {
      ...
      uint64_t value = *reinterpret_cast<const uint64_t*>(_value);
