...
zx_status_t zx_vmar_allocate(zx_handle_t parent_vmar_handle, zx_vm_option_t options,
                             uint64_t offset, uint64_t size, zx_handle_t* child_vmar,
                             zx_vaddr_t* child_addr) {
  LTRACEF("parent_vmar_handle %x, offset %#" PRIx64 ", size %#zx\n", parent_vmar_handle, offset,
          size);
  ...
  zx_status_t status =
      handle_table(up).GetDispatcherWithRights(*up, parent_vmar_handle, vmar_rights, &vmar);
  if (status != ZX_OK) {
    return status;
  }
  ...
  status = MakeAndAddHandle(ktl::move(handle), new_rights, child_vmar);
  if (status == ZX_OK) {
...
zx_status_t zx_vmar_destroy(zx_handle_t handle) {
  ...
  fbl::RefPtr<VmAddressRegionDispatcher> vmar;
  zx_status_t status =
      handle_table(up).GetDispatcherWithRights(*up, handle, ZX_RIGHT_OP_CHILDREN, &vmar);
  if (status != ZX_OK) {
    return status;
  }
...
zx_status_t vmar_map_common(zx_vm_option_t options, fbl::RefPtr<VmAddressRegionDispatcher> vmar,
  ...
  if (is_user_vmar) {
    ...
    zx_status_t status = map_result->mapping->MapRange(0, len, /*commit=*/true);
    if (status != ZX_OK) {
      return status;
    }
  }
...
zx_status_t zx_vmar_map(zx_handle_t handle, zx_vm_option_t options, uint64_t vmar_offset,
  ...
  LTRACEF("handle %x, offset %#" PRIx64 ", vmo_handle %x, len %#zx\n", handle, vmar_offset,
          vmo_handle, len);
  ...
  zx_status_t status = handle_table(up).GetDispatcherAndRights(*up, handle, &vmar, &vmar_rights);
  if (status != ZX_OK) {
    LTRACEF("bad vmar handle %x\n", handle);
    return status;
  }
  ...
  status = handle_table(up).GetDispatcherAndRights(*up, vmo_handle, &vmo, &vmo_rights);
  if (status != ZX_OK) {
    LTRACEF("bad vmo handle %x\n", vmo_handle);
    return status;
  }
...
zx_status_t zx_vmar_unmap(zx_handle_t handle, zx_vaddr_t addr, uint64_t len) {
  LTRACEF("handle %x, addr %#" PRIxPTR ", len %#zx\n", handle, addr, len);
  ProcessDispatcher* up = nullptr;
  bool is_user_thread = (ThreadDispatcher::GetCurrent() != nullptr);
  if (is_user_thread) {
    up = ProcessDispatcher::GetCurrent();
...
zx_status_t zx_vmar_protect(zx_handle_t handle, zx_vm_option_t options, zx_vaddr_t addr,
                            uint64_t len) {
  LTRACEF("handle %x, addr %#" PRIxPTR ", len %#zx\n", handle, addr, len);
  ProcessDispatcher* up = nullptr;
  bool is_user_thread = (ThreadDispatcher::GetCurrent() != nullptr);
  ...
  fbl::RefPtr<VmAddressRegionDispatcher> vmar;
  zx_status_t status =
      handle_table(up).GetDispatcherWithRights(*up, handle, vmar_rights, &vmar, &vmar_rights);
  if (status != ZX_OK) {
    return status;
  }
...
#if 0
zx_status_t zx_vmar_op_range(zx_handle_t handle, uint32_t op, zx_vaddr_t addr, uint64_t len,
