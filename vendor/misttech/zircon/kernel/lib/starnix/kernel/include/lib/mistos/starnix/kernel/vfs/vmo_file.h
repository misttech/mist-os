...
class VmoFileNode : public FsNodeOps {
 ...
 public:
  ...
  static VmoFileNode* from_vmo(zx::vmo vmo);
 ...
 private:
  VmoFileNode(zx::vmo vmo /*, MemoryXattrStorage xattrs*/)
      : vmo_(std::move(vmo)) /*, xattrs_(xattrs)*/ {}
};
...
#define fileops_impl_vmo(vmo)                                                                  \
  ...
  fit::result<Errno, zx::vmo> get_vmo(const FileObject& file, const CurrentTask& current_task, \
                                      std::optional<size_t>, ProtectionFlags prot) {           \
...
class VmoFileObject : public FileOps {
 public:
  ...
  static VmoFileObject* New(zx::vmo vmo);

  static fit::result<Errno, size_t> read(const zx::vmo& vmo, const FileObject& file, size_t offset,
                                         OutputBuffer* data);

  static fit::result<Errno, size_t> write(const zx::vmo& vmo, const FileObject& file,
                                          const CurrentTask& current_task, size_t offset,
                                          InputBuffer* data);

  static fit::result<Errno, zx::vmo> get_vmo(const zx::vmo& vmo, const FileObject&,
 ...
 private:
  VmoFileObject(zx::vmo _vmo) : vmo(std::move(_vmo)) {}
};
