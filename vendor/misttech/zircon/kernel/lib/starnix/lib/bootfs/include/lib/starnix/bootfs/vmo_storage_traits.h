...
struct StorageTraits<fbl::RefPtr<VmObject>> {
 public:
  ...
  // size.
  static fit::result<error_type, uint32_t> Capacity(const fbl::RefPtr<VmObject>& vmo);
  ...
  static fit::result<error_type> Read(const fbl::RefPtr<VmObject>& vmo, payload_type payload,
                                      void* buffer, uint32_t length);

  template <typename Callback>
  static auto Read(const fbl::RefPtr<VmObject>& vmo, payload_type payload, uint32_t length,
  ...
  static fit::result<error_type, std::optional<std::pair<fbl::RefPtr<VmObject>, uint32_t>>> Clone(
      const fbl::RefPtr<VmObject>& vmo, uint32_t offset, uint32_t length, uint32_t to_offset,
      SlopCheck&& slopcheck) {
    if (slopcheck(offset % ZX_PAGE_SIZE)) {
      return DoClone(vmo, offset, length);
    }
    return fit::ok(std::nullopt);
  }

 private:
  static fit::result<error_type> DoRead(const fbl::RefPtr<VmObject>& vmo, uint64_t offset,
  ...
  static fit::result<error_type, std::optional<std::pair<fbl::RefPtr<VmObject>, uint32_t>>> DoClone(
      const fbl::RefPtr<VmObject>& original, uint32_t offset, uint32_t length);
};
