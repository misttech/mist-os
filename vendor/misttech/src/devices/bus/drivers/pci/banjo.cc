...
#define LOG_STATUS(level, status, format, ...)                                   \
  ({                                                                             \
    ...
    LTRACEF("[%s] %s(" format ") = %s", device()->config()->addr(),              \
...
zx::result<> BanjoDevice::Create(zx_device_t* parent, pci::Device* device) {
  ...
  if (status != ZX_OK) {
    LTRACEF("[%s] Failed to create pci banjo fragment: %s\n", pci_dev->config()->addr(),
...
zx_status_t BanjoDevice::PciGetBar(uint32_t bar_id, pci_bar_t* out_bar) {
  ...
  zx::result<fbl::RefPtr<Dispatcher>> result;
  switch (bar->allocation->type()) {
    case PCI_ADDRESS_SPACE_MEMORY: {
      result = bar->allocation->CreateVmo();
      if (result.is_ok()) {
        out_bar->result.vmo = reinterpret_cast<uintptr_t>(fbl::ExportToRawPtr(&result.value()));
      }
    } break;
    case PCI_ADDRESS_SPACE_IO: {
      result = bar->allocation->CreateResource();
      if (result.is_ok()) {
        out_bar->result.io.resource =
            reinterpret_cast<uintptr_t>(fbl::ExportToRawPtr(&result.value()));
  ...
  if (result.is_error()) {
    LTRACEF("[%s] Failed to create %s for BAR %u (type = %s, range = [%#lx, %#lx)): %s\n",
...
zx_status_t BanjoDevice::PciGetBti(uint32_t index, uintptr_t* out_bti) {
  fbl::AutoLock dev_lock(device_->dev_lock());
  fbl::RefPtr<BusTransactionInitiatorDispatcher> bti;
  zx_status_t status = device_->bdi()->GetBti(device_, index, &bti);
  if (status != ZX_OK) {
    return LOG_STATUS(DEBUG, status, "%u", index);
  }
  *out_bti = reinterpret_cast<uintptr_t>(fbl::ExportToRawPtr(&bti));
  return LOG_STATUS(DEBUG, ZX_OK, "%u", index);
}
...
zx_status_t BanjoDevice::PciMapInterrupt(uint32_t which_irq, uintptr_t* out_handle) {
  zx::result<fbl::RefPtr<Dispatcher>> result = device_->MapInterrupt(which_irq);
  if (result.is_ok()) {
    *out_handle = reinterpret_cast<uintptr_t>(fbl::ExportToRawPtr(&result.value()));
  }
