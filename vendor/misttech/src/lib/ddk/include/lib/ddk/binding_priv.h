...
// instructions.
#define ZIRCON_DRIVER_PRIV(Driver, Ops, VendorName, Version, BindCount)                            \
  extern const struct mistos_driver_ldr __mistos_driver_ldr_##Driver;                              \
  const struct mistos_driver_ldr {                                                                 \
    zircon_driver_note_t note;                                                                     \
    zx_driver_rec_t rec;                                                                           \
    zx_bind_inst_t binding[BindCount];                                                             \
  } __mistos_driver_ldr_##Driver SPECIAL_SECTION(".data.rel.ro.driver", sizeof(void*))             \
      ZIRCON_DRIVER_NOTE_ASAN = {                                                                  \
          /* .note = */ {ZIRCON_DRIVER_NOTE_HEADER_INIT(__mistos_driver_ldr_##Driver),             \
                         ZIRCON_DRIVER_NOTE_PAYLOAD_INIT(Driver, VendorName, Version, BindCount)}, \
          /* .rec = */ {&(Ops), NULL}, /* .binding = */ {
#define MISTOS_DRIVER_END(Driver) \
  }                               \
  }                               \
  ;
