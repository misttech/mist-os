...
#ifndef ZXTEST_CPP_ZXTEST_KERNEL_H_
#define ZXTEST_CPP_ZXTEST_KERNEL_H_
...
#define LIB_ZXTEST_CTOR_GROUP_REGISTER 2
#define LIB_ZXTEST_CTOR_GROUP_INSTANTIATE 1
...
#define LIB_ZXTEST_REGISTER(TestCase, Test, Fixture)                                            \
  ...
  static void LIB_ZXTEST_REGISTER_FN(TestCase, Test)(uint);                                     \
  void LIB_ZXTEST_REGISTER_FN(TestCase, Test)(uint) {                                           \
  ...
  LK_INIT_HOOK(LIB_ZXTEST_TEST_CLASS(TestCase, Test), LIB_ZXTEST_REGISTER_FN(TestCase, Test),   \
               LK_INIT_LEVEL_USER - LIB_ZXTEST_CTOR_GROUP_REGISTER)                             \
...
#define LIB_ZXTEST_REGISTER_PARAMETERIZED(TestSuite, Test)                                    \
  ...
  static void LIB_ZXTEST_REGISTER_FN(TestSuite, Test)(uint);                                  \
  void LIB_ZXTEST_REGISTER_FN(TestSuite, Test)(uint) {                                        \
    fbl::AllocChecker ac;                                                                     \
    zxtest::Runner::GetInstance()->AddParameterizedTest<TestSuite>(                           \
        ktl::make_unique<zxtest::internal::AddTestDelegateImpl<                               \
            TestSuite, TestSuite::ParamType, LIB_ZXTEST_TEST_CLASS(TestSuite, Test)>>(&ac),   \
...
#define INSTANTIATE_TEST_SUITE_P(Prefix, TestSuite, ...)                                           \
  static void LIB_ZXTEST_REGISTER_FN(Prefix, TestSuite)(uint) LIB_ZXTEST_CTOR_GROUP_INSTANTIATE;   \
  void LIB_ZXTEST_REGISTER_FN(Prefix, TestSuite)(uint) {                                           \
    ...
    zxtest::Runner::GetInstance()->AddInstantiation<TestSuite, TestSuite::ParamType>(              \
        ktl::make_unique<                                                                          \
            zxtest::internal::AddInstantiationDelegateImpl<TestSuite, TestSuite::ParamType>>(&ac), \
        #Prefix, {.filename = __FILE__, .line_number = __LINE__}, provider,                        \
        LIB_ZXTEST_INSTANTIATION_NAME_FN(__VA_ARGS__));                                            \
    ASSERT(ac.check());                                                                            \
  }                                                                                                \
  LK_INIT_HOOK(Prefix##_##TestSuite, LIB_ZXTEST_REGISTER_FN(Prefix, TestSuite),                    \
               LK_INIT_LEVEL_USER - LIB_ZXTEST_CTOR_GROUP_INSTANTIATE)                             \
...
#endif  // ZXTEST_CPP_ZXTEST_KERNEL_H_
