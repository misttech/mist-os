...
springboard_t* tu_launch_init(zx_handle_t job, const char* name, int argc, const char* const* argv,
...

  {
    ...
    fidl::Status result = launcher->AddArgs(std::move(args));
  ...
  if (envp) {
    ...
    fidl::Status result = launcher->AddEnvirons(std::move(env));
...

  {
    ...
    fidl::Status result = launcher->AddNames(std::move(names));
...

  {
    ...
    fidl::Status result = launcher->AddHandles(std::move(handle_vector));
