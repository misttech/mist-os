...
// use the thread's lock. In most cases this will not be necessary but there are relatively few
...
zx_status_t arch_set_debug_regs(Thread* thread, const zx_thread_state_debug_regs_t* in) {
  ...
  // TODO(https://fxbug.dev/42108006): Ideally, we could do some querying at boot time about the
  // format that the CPU is storing reserved bits and we can create a mask we can apply to the input
