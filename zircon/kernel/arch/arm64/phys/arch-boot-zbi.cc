...
// Local labels in the inline assembly below.
extern void mmu_possibly_off_label() __asm__(".L.ZbiBoot.mmu_possibly_off");
extern void end_label() __asm__(".L.ZbiBoot.end");

// TODO(http://fxbug.dev/408020980): Remove gnu::weak when we delete the
...
[[gnu::weak]] void BootZbi::ZbiBoot(uintptr_t entry, void* data) const {
  ...
  {
    auto mmu_possibly_off = reinterpret_cast<uintptr_t>(&mmu_possibly_off_label);
    auto end = reinterpret_cast<uintptr_t>(&end_label);
    arch::CleanDataCacheRange(mmu_possibly_off, end - mmu_possibly_off);
  }
