...
static void arm64_perfmon_init_once(uint level) {
  ...
  // If we don't have an IRQ (or a usable one - https://fxbug.dev/42108266) then we can still
...
static zx_status_t arm64_perfmon_map_buffers_locked(PerfmonState* state, Guard<Mutex>& guard) {
  ...
  for (unsigned cpu = 0; cpu < num_cpus; ++cpu) {
    ...
    // TODO(https://fxbug.dev/42059713): Restructure this mapping code to performing the pinning
    // externally.
    guard.CallUntracked([&] {
...
static void arm64_perfmon_start_task(void* raw_context) TA_NO_THREAD_SAFETY_ANALYSIS {
  ...
  // TODO(https://fxbug.dev/42108266): arm64_pmu_enable_our_irq(true); - needs irq support
...
static void arm64_perfmon_stop_task(void* raw_context) TA_NO_THREAD_SAFETY_ANALYSIS {
  ...
  disable_counters();
  // TODO(https://fxbug.dev/42108266): arm64_pmu_enable_our_irq(false); - needs irq support
