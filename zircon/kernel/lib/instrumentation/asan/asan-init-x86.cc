...
void asan_remap_shadow_internal(volatile pt_entry_t* pdp, uintptr_t start, size_t size) {
  ...
  printf("KASAN: shadow for [%#" PRIxPTR ", %#" PRIxPTR ") at [%#" PRIxPTR ", %#" PRIxPTR ")\n",
         start, start + size, start_shadow, end_shadow);
  ...
  // TODO(https://fxbug.dev/42127444): When pmm_alloc_page allows getting high memory, use high
  // pages where possible for page tables and asan shadow pages.
...
void asan_map_shadow_for(uintptr_t start, size_t size) {
  // TODO(https://fxbug.dev/42104852): Allow calling map_shadow_for after early boot.
