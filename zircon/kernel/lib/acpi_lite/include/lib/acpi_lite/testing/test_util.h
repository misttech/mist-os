...
class FakePhysMemReader : public PhysMemReader {
 public:
  ...
  explicit FakePhysMemReader(zx_paddr_t rsdp, std::span<const Region> regions) : rsdp_(rsdp) {
...
class FakeAcpiParser : public AcpiParserInterface {
 public:
  ...
  FakeAcpiParser(std::initializer_list<std::span<const uint8_t>> tables) {
    for (std::span<const uint8_t> table : tables) {
