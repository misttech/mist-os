...
class AspaceEnumerator final : public VmEnumerator {
 public:
  ...
  zx_status_t OnVmAddressRegion(const VmAddressRegion* vmar, uint depth,
                                Guard<CriticalMutex>& guard) TA_REQ(vmar->lock()) override {
    if (available_ < max_) {
      zx_info_maps_t entry = {};
...
class AspaceVmoEnumerator final : public VmEnumerator {
 public:
  ...
  zx_status_t OnVmMapping(const VmMapping* map, const VmAddressRegion*, uint,
                          Guard<CriticalMutex>& guard) TA_REQ(map->lock()) override {
    if (available_ < max_) {
      zx_info_vmo_t entry =
