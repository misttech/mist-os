...
TEST(NullTest, Default) {
  ...
  uint64_t flags = 42u;
  ASSERT_STATUS(zxio_flags_get2(&io, &flags), ZX_OK);
  ASSERT_EQ(0, flags);
  ASSERT_STATUS(zxio_flags_set2(&io, flags), ZX_ERR_NOT_SUPPORTED);
  uint32_t deprecated_flags = 42u;
  ASSERT_STATUS(zxio_flags_get(&io, &deprecated_flags), ZX_OK);
  ASSERT_EQ(0, deprecated_flags);
  ASSERT_STATUS(zxio_flags_set(&io, deprecated_flags), ZX_ERR_NOT_SUPPORTED);
...
TEST(NullTest, Null) {
  ...
  ASSERT_STATUS(zxio_truncate(&io, 0u), ZX_ERR_NOT_SUPPORTED);
  uint64_t flags = 42u;
  ASSERT_STATUS(zxio_flags_get2(&io, &flags), 0);
  ASSERT_EQ(0, flags);
  ASSERT_STATUS(zxio_flags_set2(&io, flags), ZX_ERR_NOT_SUPPORTED);
  uint32_t deprecated_flags = 42u;
  ASSERT_STATUS(zxio_flags_get(&io, &deprecated_flags), 0);
  ASSERT_EQ(0, deprecated_flags);
  ASSERT_STATUS(zxio_flags_set(&io, deprecated_flags), ZX_ERR_NOT_SUPPORTED);
  zx_handle_t vmo = ZX_HANDLE_INVALID;
  ASSERT_STATUS(zxio_vmo_get_copy(&io, &vmo), ZX_ERR_NOT_SUPPORTED);
