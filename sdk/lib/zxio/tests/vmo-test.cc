...
TEST_F(VmoTest, Basic) {
  ...
  uint64_t flags = 0u;
  ASSERT_STATUS(ZX_OK, zxio_flags_get2(io, &flags));
  ...
  ASSERT_STATUS(ZX_ERR_NOT_SUPPORTED, zxio_flags_set2(io, flags));

  uint32_t deprecated_flags = 0u;
  ASSERT_STATUS(ZX_OK, zxio_flags_get(io, &deprecated_flags));
  EXPECT_TRUE(
  ...
  ASSERT_STATUS(ZX_ERR_NOT_SUPPORTED, zxio_flags_set(io, deprecated_flags));
