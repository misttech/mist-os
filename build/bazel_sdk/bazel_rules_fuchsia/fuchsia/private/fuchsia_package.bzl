...
def _fuchsia_test_package(
    ...
    _build_fuchsia_package_test(
        ...
        target_compatible_with = target_compat,
        tags = tags + ["manual"],
        ignore_autogen_cml = ignore_autogen_cml,
        **kwargs
    )
...
def _build_fuchsia_package_impl(ctx):
    ...
    for r in ctx.attr.collected_resources[FuchsiaCollectedPackageResourcesInfo].collected_resources.to_list():
        #TODO(356932372) we need to ignore the autogenerated components when we
        # are building a fuchsia_test_package instead of a fuchsia_unittest_package
        # because the fuchsia_wrap_cc_test always put the autogenerated component
        # in the build graph cuasing the cml file to get picked up.
        if ctx.attr.ignore_autogen_cml and r.src.basename.endswith("_autogen_cml.cm"):
            continue
        package_resources.append(r)
...
_build_fuchsia_package, _build_fuchsia_package_test = rule_variants(
    ...
    attrs = {
        ...
        "ignore_autogen_cml": attr.bool(
            doc = """A temporary boolean which lets us ignore autogenerated cml files.

            This attribute is temporary and will be removed when 356932372 is resolved.
            """,
            default = True,
        ),
