...
# Location of template that will be expanded into the top-level BUILD.bazel file.
# Note that the resulting file will then be modified multiple times by functions in this file.
_REPOSITORY_BUILD_TEMPLATE = "//fuchsia/workspace/sdk_templates:repository.BUILD.template"
...
def resolve_repository_labels(ctx):
    ...
    """
    ctx.path(Label(_REPOSITORY_BUILD_TEMPLATE))
...
def _sdk_id_from_manifests(ctx, manifests):
...
def _generate_sdk_constants(repo_ctx, manifests):
...
def _generate_sdk_build_rules(ctx, manifests, copy_content_strategy, constants, filter_types = None, exclude_types = None):
...
def _export_all_files(ctx):
    ...
    ctx.template(
        ...
        "BUILD.bazel",
        substitutions = {
            "{{ALL_FILES}}": _serialize(exported_targets),
        },
        executable = False,
    )

def generate_sdk_repository(ctx, manifests, copy_content_strategy, run_buildifier):
    """Generate the content of the Fuchsia Bazel SDK repository.
    ...
    Args:
        ...
        copy_content_strategy: "symlink" to create symlinks to Fuchsia SDK artifacts or "copy" to attempt
            to create hardlinks, or standard copy if hardlinks are not possible
        run_buildifier: An optional function used to run the "buildifier" tool over the
            generated repository, to fix and verify formatting. This must take two
            (ctx, buildifier_flags) arguments, where 'ctx' is the repository context,
            and 'buildifier_flags' is a list of string passed as command-line flags
            to the tool.
    """

    # Extract the target CPU names supported by our SDK manifests, then
    # write it to generated_constants.bzl file.
    constants = _generate_sdk_constants(ctx, manifests)
    ...
    ctx.template(
        ...
        substitutions = {
            ...
            "{{SDK_ID}}": _sdk_id_from_manifests(ctx, manifests),
        },
        executable = False,
    )
    ...
    _generate_sdk_build_rules(ctx, manifests, copy_content_strategy, constants)
    ...
    if run_buildifier:
        ...
        if not run_buildifier(ctx, ["-mode=fix", "-lint=fix", "-r", "."]):
        ...
        if not run_buildifier(ctx, ["-mode=fix", "-lint=warn", "-r", "."]):
