...
def generate_bazel_content_hash_files(
    fuchsia_dir: Path, output_dir: Path, bazel_content_hashes: T.Any
) -> T.Set[Path]:
    ...
    for entry in bazel_content_hashes:
        ...
        for source_path in entry["source_paths"]:
            assert source_path.startswith(
                "//"
            ), f"Unexpected source path in bazel_content_hashes.json: {source_path}"
            fstate.hash_source_path(fuchsia_dir / source_path[2:])
